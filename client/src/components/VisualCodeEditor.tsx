...
export function VisualCodeEditor({ onCodeChange, onExecute }: VisualCodeEditorProps): React.ReactElement {
    ...
    const [draggedPlacedBlock, setDraggedPlacedBlock] = useState<{
        id: number;
        offsetX: number;
        offsetY: number;
    } | null>(null);
    ...
    const updateBlockData = (blockId: number, newData: Partial<CreateMindData & CreateMentalData & SetAttributeData & AddMentalData> | Record<string, any>) => {
        setBlocks((prev) => {
            const next = prev.map((block) => block.id === blockId ? ({ ...block, data: { ...(block as any).data, ...newData } } as Block) : block);
            emitCodeChange(next);
            return next;
        });
    };
    ...
    const connectBlocks = (topBlockId: number, bottomBlockId: number) => {
        setBlocks((prev) => {
            ...
            while (currentBlockId) {
                ...
                if (!currentBlock)
                    break;
                updated = updated.map((block) => block.id === currentBlockId
    ...
    const findSnapTarget = (draggedBlockId: number, x: number, y: number, currentBlocks: Block[]): {
        id: number;
        side: 'top' | 'bottom';
    } | null => {
        ...
        if (!bestTarget)
            return null;
        return { id: (bestTarget as {
                id: number;
                side: 'top' | 'bottom';
                distance: number;
            }).id, side: (bestTarget as {
                id: number;
                side: 'top' | 'bottom';
                distance: number;
            }).side };
    ...
    React.useEffect(() => {
        ...
        const handleGlobalMouseMove = (e: MouseEvent) => {
            ...
            setBlocks((prev) => {
                ...
                let updated = prev.map((block) => block.id === draggedPlacedBlock.id ? ({ ...block, x: finalX, y: finalY } as Block) : block);
        ...
        switch (block.type) {
            case 'create_mind':
                return (<div key={block.id} ref={(el) => {
                    ...
                    }} className={blockClassName} style={{ left: block.x, top: block.y }} onClick={() => setSelectedBlock(block)} onMouseDown={(e) => handleBlockMouseDown(e, block)}>
            <StarterJigsaw color={block.color}>
              <div className="block-content">
                <input type="text" value={block.data.variableName} onChange={(e) => updateBlockData(block.id, { variableName: e.target.value })} className="block-input" onClick={(e) => e.stopPropagation()}/>
                <span> = Mind()</span>
              </div>
            </StarterJigsaw>
            ...
            <StarterJigsaw color={block.color}>
              <div className="block-content attribute-content">
                <div className="attribute-row" style={{ marginBottom: '6px' }}>
                  <input type="text" value={block.data.target} onChange={(e) => updateBlockData(block.id, { target: e.target.value })} className="block-input small" onClick={(e) => e.stopPropagation()} placeholder="var"/>
                </div>
                <div className="attribute-row">
                  <input type="checkbox" checked={block.data.enableColor} onChange={(e) => updateBlockData(block.id, { enableColor: e.target.checked })} onClick={(e) => e.stopPropagation()} style={{ width: '14px', height: '14px', marginRight: '4px', cursor: 'pointer' }}/>
                  <span style={{ opacity: block.data.enableColor ? 1 : 0.5 }}>.color = </span>
                  <input type="color" value={block.data.color} onChange={(e) => updateBlockData(block.id, { color: e.target.value })} className="block-input block-input-color" onClick={(e) => e.stopPropagation()} disabled={!block.data.enableColor} style={{ opacity: block.data.enableColor ? 1 : 0.5 }}/>
                </div>
                <div className="attribute-row">
                  <input type="checkbox" checked={block.data.enableName} onChange={(e) => updateBlockData(block.id, { enableName: e.target.checked })} onClick={(e) => e.stopPropagation()} style={{ width: '14px', height: '14px', marginRight: '4px', cursor: 'pointer' }}/>
                  <span style={{ opacity: block.data.enableName ? 1 : 0.5 }}>.name = </span>
                  <input type="text" value={block.data.name} onChange={(e) => updateBlockData(block.id, { name: e.target.value })} className="block-input" onClick={(e) => e.stopPropagation()} disabled={!block.data.enableName} style={{ opacity: block.data.enableName ? 1 : 0.5 }} placeholder="name"/>
                </div>
                <div className="attribute-row">
                  <input type="checkbox" checked={block.data.enableScale} onChange={(e) => updateBlockData(block.id, { enableScale: e.target.checked })} onClick={(e) => e.stopPropagation()} style={{ width: '14px', height: '14px', marginRight: '4px', cursor: 'pointer' }}/>
                  <span style={{ opacity: block.data.enableScale ? 1 : 0.5 }}>.scale = </span>
                  <input type="text" value={block.data.scale} onChange={(e) => updateBlockData(block.id, { scale: e.target.value })} className="block-input" onClick={(e) => e.stopPropagation()} disabled={!block.data.enableScale} style={{ opacity: block.data.enableScale ? 1 : 0.5 }} placeholder="1.0"/>
                </div>
              </div>
            </StarterJigsaw>
            <button className="block-delete" onClick={() => deleteBlock(block.id)}>
            ...
            <StarterJigsaw color={block.color}>
              <div className="block-content">
                <input type="text" value={block.data.mindVar} onChange={(e) => updateBlockData(block.id, { mindVar: e.target.value })} className="block-input small" onClick={(e) => e.stopPropagation()}/>
                <span>.add(</span>
                <input type="text" value={block.data.mentalVar} onChange={(e) => updateBlockData(block.id, { mentalVar: e.target.value })} className="block-input small" onClick={(e) => e.stopPropagation()}/>
                <span>)</span>
              </div>
            </StarterJigsaw>
            <button className="block-delete" onClick={() => deleteBlock(block.id)}>
    ...
    return (<div className="visual-code-editor">
      <div className="block-palette">
        <h3>Blocks</h3>
        <div className="block-templates">
          {blockTemplates.map((template) => (<div key={template.type} className="block-template" draggable onDragStart={(e) => handleDragStart(e, template)} style={{ ['--template-color' as any]: template.color }}>
              {template.label}
            </div>))}
        </div>
        <button className="examples-button" onClick={() => setShowExamples(!showExamples)} style={{
            marginBottom: '10px',
      ...
      {showExamples && (<div className="examples-panel">
          <h3 style={{ marginBottom: '15px', color: '#00d4ff' }}>Code Examples</h3>
          <div className="examples-grid">
            {CODE_EXAMPLES.map((example) => (<div key={example.id} className="example-card" onClick={() => loadExample(example)} style={{
      ...
      <div ref={editorRef} className="block-editor" onDrop={handleDrop} onDragOver={handleDragOver} onClick={() => setSelectedBlock(null)}>
