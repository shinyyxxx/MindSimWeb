...
function ProfileModelView({ modelPath, markers }: {
    modelPath: string;
    markers: Marker[];
}) {
    ...
    useEffect(() => {
        ...
        }, undefined, (err) => console.error('Failed to load profile model', err));
        return () => {
            mounted = false;
        };
    }, [modelPath]);
    return (<Canvas camera={{ position: [0, 0.55, 0.8], fov: 45 }} style={{ width: '100%', height: '100%' }} gl={{ antialias: true }}>
      ...
      {markers.map((m) => (<mesh key={m.key} position={[m.position.x, m.position.y, m.position.z]} castShadow={false} receiveShadow={false}>
...
function ProfilePanel({ profile, attrs, markers, attrKey, attrValue, onAttrKeyChange, onAttrValueChange, onAddAttribute, onClose, }: ProfilePanelProps) {
    ...
    return (<div className="profile-panel">
      ...
      <div className="profile-panel__body">
        <div className="profile-panel__model">
          {profile.modelPath ? (isPerception ? (<ProfileModelView modelPath={profile.modelPath} markers={markers}/>) : (<model-viewer src={profile.modelPath} auto-rotate camera-controls style={{ width: '100%', height: '100%', background: 'transparent' }}/>)) : (<div className="profile-panel__placeholder">No model available</div>)}
        </div>
        <div className="profile-panel__attrs">
          ...
          {profile.labelNumber && (<div className="profile-panel__attr"><strong>Label #:</strong> {profile.labelNumber}</div>)}
          {isPerception && (<>
              ...
              <div className="profile-panel__chips">
                ...
                {attrs.map((attr) => (<span key={attr.key} className="profile-panel__chip">
                    <span className="profile-panel__chip-dot"/>
                    <span>{attr.key}: {attr.value || '—'}</span>
                  </span>))}
              </div>
              <div className="profile-panel__form">
                <input className="profile-panel__input" placeholder="Key" value={attrKey} onChange={(e) => onAttrKeyChange(e.target.value)}/>
                <input className="profile-panel__input" placeholder="Value" value={attrValue} onChange={(e) => onAttrValueChange(e.target.value)}/>
