...
export function SamskaraDemo(): React.ReactElement {
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < ballCount; i++) {
            ...
            root.add(mesh);
            const vel = new THREE.Vector3((Math.random() * 2 - 1) * 0.45, (Math.random() * 2 - 1) * 0.45, (Math.random() * 2 - 1) * 0.45);
            balls.push({ mesh, vel });
        }
        ...
        function step(t: number) {
            ...
            for (const b of balls) {
                ...
                if (sp > maxSpeed)
                    b.vel.multiplyScalar(maxSpeed / sp);
                if (sp < minSpeed) {
                    b.vel
                        .copy(tmp
                ...
                if (dist > limit) {
                    ...
                    if (vn > 0)
                        b.vel.copy(reflectVelocity(b.vel, normal, restitution));
                    if (b.vel.dot(normal) > -0.08)
                        b.vel.addScaledVector(normal, -0.12);
                }
            }
    ...
    return (<div className="skandha-3d">
