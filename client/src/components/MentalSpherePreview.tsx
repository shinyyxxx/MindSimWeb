...
function MentalSphereScene({ card, accentColor }: { card: CetasikaCard; accentColor: string }) {
  ...
  useLayoutEffect(() => {
    ...
    return () => {
      ...
      if (m && g) g.remove(m)
      if (mentalRef.current?.dispose) mentalRef.current.dispose()
...
export function MentalSpherePreview({
  card,
  accentColor,
}: {
  card: CetasikaCard
