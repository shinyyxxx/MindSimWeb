...
export function DuckPool({ className, style, onDropDropletReady, allowUserDrops = false, }: {
    className?: string;
    ...
    useEffect(() => {
        ...
        let unmounted = false;
        async function init() {
            const [THREE, { instanceIndex, uint, int, float, length, clamp, vec2, cos, vec3, vertexIndex, Fn, uniform, instancedArray, min, max, positionLocal, transformNormalToView, select, globalId }, { Inspector }, { SimplexNoise }, { HDRLoader }, { OrbitControls }, WebGPUCapability,] = await Promise.all([
                import('three/webgpu'),
                import('three/tsl'),
            ...
            const getNeighborValuesTSL = (index: ReturnType<typeof TSL.uint>, store: ReturnType<typeof heightStorageA>) => {
            ...
            const createComputeColor = (readR: ReturnType<typeof colorRStorageA>, readG: ReturnType<typeof colorGStorageA>, readB: ReturnType<typeof colorBStorageA>, writeR: ReturnType<typeof colorRStorageB>, writeG: ReturnType<typeof colorGStorageB>, writeB: ReturnType<typeof colorBStorageB>) => TSL.Fn(() => {
                ...
                const getHeightAt = (idx: ReturnType<typeof TSL.uint>) => TSL.select(readFromA, heightStorageA.element(idx), heightStorageB.element(idx));
                const diffuseChannel = (readStore: ReturnType<typeof colorRStorageA>, writeStore: ReturnType<typeof colorRStorageB>) => {
                    ...
                    const wN = TSL.clamp(base.sub(velY.mul(advectionStrength).mul(bias)), TSL.float(0.08), TSL.float(0.32));
                    const wS = TSL.clamp(base.add(velY.mul(advectionStrength).mul(bias)), TSL.float(0.08), TSL.float(0.32));
                    const wE = TSL.clamp(base.sub(velX.mul(advectionStrength).mul(bias)), TSL.float(0.08), TSL.float(0.32));
                    const wW = TSL.clamp(base.add(velX.mul(advectionStrength).mul(bias)), TSL.float(0.08), TSL.float(0.32));
                    ...
                    writeStore.element(index).assign(c.mul(colorDiffusionRate).add(diffusion.mul(TSL.float(1.0).sub(colorDiffusionRate))));
                };
                ...
                const rawInfluence = TSL.clamp(TSL.float(1.0).sub(dist.div(dropletSize)), TSL.float(0.0), TSL.float(1.0));
            ...
            const computeColorAtoB = createComputeColor(colorRStorageA, colorGStorageA, colorBStorageA, colorRStorageB, colorGStorageB, colorBStorageB).setName('Update Color A→B');
            const computeColorBtoA = createComputeColor(colorRStorageB, colorGStorageB, colorBStorageB, colorRStorageA, colorGStorageA, colorBStorageA).setName('Update Color B→A');
            const createComputeHeight = (readBuffer: ReturnType<typeof heightStorageA>, writeBuffer: ReturnType<typeof heightStorageB>) => TSL.Fn(() => {
                ...
                const mousePhase = TSL.clamp(TSL.length(TSL.vec2(x, y).sub(centerVec).mul(BOUNDS).sub(mousePos))
                    .mul(Math.PI)
                    .div(mouseSize), 0.0, Math.PI);
                newHeight.addAssign(TSL.cos(mousePhase).add(1.0).mul(mouseDeep).mul(mouseSpeed.length()));
            ...
            const getCurrentHeight = (index: ReturnType<typeof TSL.uint>) => TSL.select(readFromA, heightStorageA.element(index), heightStorageB.element(index));
            const getCurrentColor = (index: ReturnType<typeof TSL.uint>) => TSL.vec3(TSL.select(readFromColorA, colorRStorageA.element(index), colorRStorageB.element(index)), TSL.select(readFromColorA, colorGStorageA.element(index), colorGStorageB.element(index)), TSL.select(readFromColorA, colorBStorageA.element(index), colorBStorageB.element(index)));
            ...
            waterMaterial.normalNode = TSL.Fn(() => {
                ...
                return TSL.transformNormalToView(TSL.vec3(normalX, normalY.negate(), 1.0)).toVertexStage();
            })();
            ...
            waterMaterial.positionNode = TSL.Fn(() => TSL.vec3(TSL.positionLocal.x, TSL.positionLocal.y, getCurrentHeight(TSL.vertexIndex)))();
            ...
            scene.add(meshRay);
            const hdrLoader = new HDRLoader().setPath(ASSETS.hdr.replace(/[^/]+$/, '') || 'https://threejs.org/examples/textures/equirectangular/');
            ...
            function setMouseCoords(x: number, y: number) {
                mouseCoords.set((x / (webgpuRenderer.domElement as HTMLCanvasElement).clientWidth) * 2 - 1, -((y / (webgpuRenderer.domElement as HTMLCanvasElement).clientHeight) * 2 - 1));
            }
            ...
            function raycast() {
                if (allowUserDrops && mouseDown && (firstClick || !controls.enabled)) {
                    ...
                    if (intersects.length > 0) {
                        ...
                        effectController.injectDroplet.value = 1;
                        effectController.mouseSpeed.value.set(point.x - effectController.mousePos.value.x, point.z - effectController.mousePos.value.y);
                        effectController.mousePos.value.set(point.x, point.z);
                        if (firstClick)
        ...
        init().then((fn) => {
            if (unmounted && typeof fn === 'function')
                fn();
            else
                cleanupRef.current = typeof fn === 'function' ? fn : null;
        });
        return () => {
            unmounted = true;
            cancelled = true;
