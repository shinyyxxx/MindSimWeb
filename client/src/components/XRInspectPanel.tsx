...
import {
  ...
  type StationaryXrPanelLayout,
} from '../pages/simulation/useStationaryDraggableXrPanel'

type XRInspectAction = 'detail' | 'back' | 'profile' | 'close' | 'voice' | 'how'

/** Mind Study Inspect desktop OptionMenu — colors + XR mesh approximations of CSS gradients. */
const XR_MENU = {
  cardRim: 0x4d7ab8,
  headerMuted: '#93c5fd',
  title: '#f8fafc',
...
/**
 * Desktop menu gradient — must be fully opaque on the whole canvas. Clipping + rgba leaves
 * transparent texels; ShapeGeometry UVs can sample those and XR shows a black “smear” (often lower-left).
 */
...
function drawOptionMenuIcon(ctx: CanvasRenderingContext2D, size: number, kind: OptionMenuIconKind): void {
  ...
  const w = size - pad * 2
  const rad = 16
  const rr = (ctx as CanvasRenderingContext2D & { roundRect?: (...a: number[]) => void }).roundRect
...
type XRInspectPanelProps = {
  ...
  onClose: () => void
  /** When true, Voice row is dimmed (matches desktop OptionMenu). */
  ...
  /** Override default XR placement (`forward` / `right` / `yDown`); lower `yDown` raises the panel. */
  stationaryLayout?: Partial<StationaryXrPanelLayout>
  /**
   * Uniform scale for panel meshes only. Must be applied here, not on a parent of `groupRef`, or
   * world placement from `useStationaryDraggableXrPanel` is multiplied by the parent scale and sits too low.
   */
  contentScale?: number
}
...
export function XRInspectPanel({
  ...
  /** Narrow card: option width ~ longest subtitle + icon + padding. */
  const optH = 0.114
  const optW = 0.72
  const iconS = 0.052
  const cardW = 0.8
  ...
  const rimGeo = useMemo(
    () =>
      createRoundedShapeGeometry(
        cardW + rimPad * 2,
        (inspectOpen ? cardHDetail : cardHOptions) + rimPad * 2,
        0.07,
      ),
    [inspectOpen],
  )
  ...
  const cardGeo = useMemo(
    () => createRoundedShapeGeometry(cardW, inspectOpen ? cardHDetail : cardHOptions, 0.072),
    [inspectOpen],
  )
  ...
  return (
    <group ref={groupRef}>
      <group scale={contentScale}>
        {/* Soft outer rim (desktop menu border glow) */}
        <mesh position={[0, 0, -0.003]} geometry={rimGeo}>
          <meshBasicMaterial
            color={XR_MENU.cardRim}
            transparent
            opacity={0.32}
            depthWrite={false}
        ...
        {!inspectOpen && (
          <>
            <Text
              position={[-0.36, 0.268, 0.004]}
              anchorX="left"
              anchorY="middle"
              fontSize={0.0155}
              letterSpacing={0.06}
              color={XR_MENU.bodyDim}
            >
              MENTAL SPHERE
            </Text>
            ...
            <mesh ref={closeButtonRef} position={[0.33, 0.238, 0.006]} geometry={closeGeo}>
              <meshBasicMaterial
                color={hoveredAction === 'close' ? XR_MENU.closeBgHover : XR_MENU.closeBg}
                transparent
                opacity={0.92}
                depthWrite={false}
                side={THREE.DoubleSide}
            ...
            {/* View Detail — row centers spaced by > optH to avoid overlap */}
            ...
            <Text
              position={[-0.19, -0.045, 0.008]}
              anchorX="left"
              anchorY="middle"
              fontSize={0.03}
              maxWidth={0.48}
              color={voiceLoading ? '#94a3b8' : XR_MENU.title}
            >
              Voice
            </Text>
            <Text
              position={[-0.19, -0.077, 0.008]}
              anchorX="left"
              anchorY="middle"
              fontSize={0.02}
              maxWidth={0.48}
        ...
        {inspectOpen && (
          <>
            ...
            <mesh ref={backButtonRef} position={[-0.13, -0.352, 0.006]} geometry={backGeo}>
              <meshBasicMaterial
                color={hoveredAction === 'back' ? XR_MENU.closeBgHover : XR_MENU.closeBg}
                transparent
                opacity={0.95}
                depthWrite={false}
                side={THREE.DoubleSide}
            ...
            <mesh ref={profileButtonRef} position={[0.15, -0.352, 0.006]} geometry={profileGeo}>
              <meshBasicMaterial
                color={hoveredAction === 'profile' ? 0x0284c7 : 0x0ea5e9}
                transparent
                opacity={0.95}
                depthWrite={false}
                side={THREE.DoubleSide}
