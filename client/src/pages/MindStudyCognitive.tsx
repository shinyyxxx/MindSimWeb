...
type MentalSeed = {
    ...
    type?: 'perception';
    variant?: 'good' | 'bad' | 'greed' | 'hatred' | 'delusion' | 'wrong_view' | 'conceit' | 'doubt' | 'restlessness' | 'shamelessness' | 'recklessness' | 'sloth' | 'torpor' | 'worry' | 'envy' | 'stinginess' | 'neutral' | 'perception' | 'contact' | 'feeling' | 'intention' | 'attention' | 'concentration' | 'life_faculty';
};
function HumanBody({ mind, controlsRef, url = `${import.meta.env.BASE_URL}assets/humanMind/human.gltf`, targetHeight = 6, groundY = -2, bodyOpacity = 0.02, mindYOffsetWorld = 0.9, humanZOffsetWorld = 0.12, mindScale: mindScaleProp = 0.35, }: {
    mind: Mind;
    ...
    return (<group position={[humanPosition.x, humanPosition.y, humanPosition.z]} scale={scaleFactor}>
      <primitive object={humanScene}/>
    </group>);
}
function MindSphere({ mind, selectedMentalName, focusTargetRef, }: {
    mind: Mind;
    ...
    useFrame((_state, delta) => {
        ...
        if (selectedMentalName && focusTargetRef.current) {
            ...
            const approachDir = new THREE.Vector3(0, 0.2, 1).normalize();
...
function MentalsLayer({ mind, mentals, selectedMentalName, onSelectMental, focusTargetRef, planeModelPath, sendMode, onSendSelection, onHoverSelection, onSendMeshSelection, }: {
    mind: Mind;
    mentals: Mental[];
    selectedMentalName: string | null;
...
function ThreeScene({ mind, mentals, selectedMentalName, onSelectMental, onUpdatePanelPosition, sendMode, onSendSelection, }: {
    mind: Mind;
    mentals: Mental[];
    selectedMentalName: string | null;
    onSelectMental: (info: InspectSelection) => void;
    ...
    return (<Canvas camera={{ position: [0, 0, 10], fov: 75 }} shadows gl={{ antialias: true, toneMappingExposure: 0.6 }}>
      <Environment preset="dawn" background blur={1} backgroundIntensity={0.35} environmentIntensity={0.6}/>
      <OrbitControls ref={controlsRef} enableDamping={!selectedMentalName} dampingFactor={selectedMentalName ? 0 : 0.05} enableZoom enablePan={!selectedMentalName} enableRotate={!selectedMentalName} minDistance={0.5} maxDistance={24} target={[mind.position.x, mind.position.y, mind.position.z]}/>
      ...
      <MentalsLayer mind={mind} mentals={mentals} selectedMentalName={selectedMentalName} onSelectMental={onSelectMental} focusTargetRef={focusTargetRef} planeModelPath={paperPlaneModel} sendMode={sendMode} onSendSelection={onSendSelection} onHoverSelection={setHoverSelection} onSendMeshSelection={setSendMeshSelection}/>
...
function OptionMenu({ selection, panelPosition, onClose, onVoice, voiceLoading, onViewDetail, }: {
...
export function MindStudyCognitive(): React.ReactElement {
    ...
    return (<main className="page simulation-page">
      <div className="simulation-full" style={{ position: 'relative' }}>
        ...
        {selected && !inspectOpen && (<OptionMenu selection={selected} panelPosition={panelPosition} onClose={handleClose} onVoice={handleVoice} voiceLoading={voiceLoading} onViewDetail={handleViewDetail}/>)}
        {selected && inspectOpen && (<InspectPanel selection={selected} panelPosition={panelPosition} onClose={handleClose} onShowProfile={handleShowProfile}/>)}
        {profile && (<ProfilePanel profile={profile} attrs={profileAttrs} markers={profileMarkers} attrKey={attrKey} attrValue={attrValue} onAttrKeyChange={setAttrKey} onAttrValueChange={setAttrValue} onAddAttribute={handleAddAttribute} onClose={handleCloseProfile}/>)}
        <ThreeScene mind={mind} mentals={mentals} selectedMentalName={selected?.name ?? null} onSelectMental={handleSelect} onUpdatePanelPosition={setPanelPosition} sendMode={sendMode} onSendSelection={setSendInfo}/>
      </div>
    </main>);
}
