...
function MentalsLayer({
  ...
  useEffect(() => {
    ...
    next.forEach((mental) => {
      ...
      if (hasHydratedMentalsRef.current) {
        const isGreenOrRed = mental.color === 0x22c55e || mental.color === 0xef4444
        const targetScale = mental.scale * (isGreenOrRed ? mentalScaleFactor : 1)
  ...
  useEffect(() => {
    enteringMentalsRef.current.forEach((anim, mental) => {
      const isGreenOrRed = mental.color === 0x22c55e || mental.color === 0xef4444
      anim.targetScale = mental.scale * (isGreenOrRed ? mentalScaleFactor : 1)
    })
    appliedMentalsRef.current.forEach((mental) => {
      if (enteringMentalsRef.current.has(mental)) return
      const isGreenOrRed = mental.color === 0x22c55e || mental.color === 0xef4444
      if (!isGreenOrRed) return
      const mesh = mental.getMesh()
      if (!mesh) return
      const s = mental.scale * mentalScaleFactor
                      ...
                      return (
                        <button
                          ...
                          onClick={() => {
                            ...
                            onSenseSelect?.(selectedSense, v.params)
                          }}
                          style={{
                            display: 'flex',
                            alignItems: 'center',
                            ...
                            background: isActive ? 'rgba(59, 130, 246, 0.25)' : 'rgba(30, 41, 59, 0.65)',
...
function ThreeScene({
  ...
  const mentalScaleFactor = useMemo(() => {
    if (!vithiStageData || vithiStageData.size === 0) return 1
    const stage = vithiStageData.get(timelineIndex)
    if (!stage || stage.blocked || stage.events.length <= 1) return 1
    const stageName = VITHI_STAGE_ORDER[timelineIndex]
    if (stageName !== 'javana' && stageName !== 'tadalammana') return 1
    ...
    >
      ...
      {showMentalsLayer && (
        <MentalsLayer
          ...
          mentalScaleFactor={mentalScaleFactor}
        />
      )}
