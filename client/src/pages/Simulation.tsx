...
const SENSE_VARIANTS: Record<string, {
    id: string;
    label: string;
    icon: string;
    params: VithiParams;
...
type MentalSeed = {
    ...
    type?: 'perception';
    variant?: 'faith' | 'mindfulness' | 'moral_shame' | 'moral_dread' | 'non_greed' | 'non_hatred' | 'equanimity' | 'appreciative_joy' | 'tranquility_body' | 'tranquility_mind' | 'lightness_body' | 'lightness_mind' | 'pliancy_body' | 'pliancy_mind' | 'wieldiness_body' | 'wieldiness_mind' | 'proficiency_body' | 'proficiency_mind' | 'rectitude_body' | 'rectitude_mind' | 'bad' | 'greed' | 'hatred' | 'delusion' | 'wrong_view' | 'conceit' | 'doubt' | 'restlessness' | 'shamelessness' | 'recklessness' | 'sloth' | 'torpor' | 'worry' | 'envy' | 'stinginess' | 'neutral' | 'perception' | 'contact' | 'feeling' | 'intention' | 'attention' | 'concentration' | 'life_faculty' | 'initial_application' | 'sustained_application' | 'decision' | 'energy' | 'desire' | 'rapture' | 'right_speech' | 'right_action' | 'right_livelihood' | 'compassion' | 'appreciative_joy' | 'wisdom';
};
...
const T5_MENTAL_OPTIONS: {
    id: string;
    label: string;
    variants: MentalVariant[];
}[] = [
...
function resolveSeedForVariant(variant: MentalVariant): MentalSeed {
    ...
    if (fromT0Specific)
        return fromT0Specific;
    if (variant === 'rapture')
        return RAPTURE_TIMELINE_SEED;
    const fromOtherPools = getSeedForVariant(variant);
    if (fromOtherPools)
        return fromOtherPools;
    return {
        name: variant,
        color: '#9ca3af',
        scale: 0.12,
    ...
    if (index === 0) {
        ...
        if (!t5SelectedId) {
            ...
            indices.forEach((i) => {
                ...
                if (variant)
                    pushUnique(variant as MentalVariant);
            });
        }
    }
    else {
...
function generateCodeRunnerDslForTimelinePick(pick: Pick<TimelineScriptPick, 'timelineIndex' | 't3Happy' | 't5SelectedId'>, sceneMind: {
    getName: () => string;
    color: number;
    scale: number;
}): string {
    ...
    variants.forEach((variant, index) => {
        ...
        try {
            ...
            const ctorName = (mental as unknown as {
                constructor?: {
                    name?: string;
                };
            ...
            lines.push(`${varName}.color = ${dslStringifyCodeText(dslToHexColor(`#${(mental.color & 0xffffff).toString(16).padStart(6, '0')}`))}`);
...
function buildAllTimelineScriptsBundle(sceneMind: {
    getName: () => string;
    color: number;
    scale: number;
}): string {
...
function samplePointsFromObj(root: THREE.Object3D, count: number, targetHeight: number): Float32Array {
    ...
    if (!samplers.length || totalWeight <= 0)
        return sampleSpherePoints(count, targetHeight * 0.2);
...
function HumanBody({ mind, controlsRef, selectedShape = 'human', targetHeight = 2.25, groundY = DEFAULT_HUMAN_GROUND_Y, mindWorldScale = 0.1, mindYOffsetWorld = 0.26, mindZOffsetWorld = -0.06, humanZOffsetWorld = -0.6, }: {
    mind: Mind;
    ...
    const anchorWorld = useMemo(() => new THREE.Vector3(0, groundY, humanZOffsetWorld), [groundY, humanZOffsetWorld]);
    useLayoutEffect(() => {
        ...
        mind.setPosition(anchorWorld.x + chest.x, anchorWorld.y + chest.y + selectedYOffset, anchorWorld.z + chest.z + selectedZOffset);
        ...
        if (ctl) {
            ...
            ctl.target.set(targetX, targetY, targetZ);
    ...
    return (<group position={[anchorWorld.x, anchorWorld.y, anchorWorld.z]}>
      <points ref={pointsRef} frustumCulled={false}>
        <bufferGeometry ref={geometryRef}>
          <bufferAttribute attach="attributes-position" args={[currentPositionsRef.current, 3]}/>
        </bufferGeometry>
        <pointsMaterial color={0xd7e0f2} size={0.026} transparent opacity={0.38} depthWrite={false} depthTest map={spriteTexture} alphaMap={spriteTexture} alphaTest={0.02} blending={THREE.NormalBlending} sizeAttenuation/>
      </points>
    </group>);
}
...
function MentalsLayer({ mind, mentals, selectedMentalName, onSelectMental, controlsRef, focusTargetRef, planeModelPath, sendMode, emojiMode, blockXrMentalPick = false, onSendSelection, onHoverSelection, onSendMeshSelection, }: {
    mind: Mind;
    mentals: Mental[];
    ...
    const enteringMentalsRef = useRef<Map<Mental, {
        elapsed: number;
        duration: number;
        startScale: number;
        targetScale: number;
    ...
    const handleMentalPick = useCallback((found: Mental, screenPos?: {
        x: number;
        y: number;
    }) => {
        if (sendMode) {
    ...
    useEffect(() => {
        ...
        const handlePointerUp = () => {
            ...
            const worldDistance = Math.sqrt(position.x * position.x +
                position.y * position.y +
                position.z * position.z) * mind.scale;
    ...
    useEffect(() => {
        ...
        const handleXrSelect = (event: unknown) => {
            if (!gl.xr.isPresenting)
                return;
            if (blockXrMentalPick)
                return;
            if (selectedMentalName && !sendMode && !emojiMode)
                return;
            const controller = (event as {
                target?: unknown;
            }).target as THREE.Object3D | undefined;
            if (!controller)
                return;
            const list = mind.getMentals();
            const targets = collectMentalTargets(list);
    ...
    useFrame(() => {
        if (!onHoverSelection || sendMode || !gl.xr.isPresenting)
            return;
        const list = mind.getMentals();
        const targets = collectMentalTargets(list);
        if (!targets.length) {
    ...
    return (<>
      {bursts.map((burst) => (<points key={burst.id} frustumCulled={false} ref={(node) => {
                if (node)
                    burstPointRefs.current.set(burst.id, node);
                else
                    burstPointRefs.current.delete(burst.id);
            }}>
          <bufferGeometry>
            <bufferAttribute attach="attributes-position" args={[burst.positions, 3]}/>
          </bufferGeometry>
          <pointsMaterial color={burst.color} size={burst.size} sizeAttenuation map={particleSpriteTexture} alphaMap={particleSpriteTexture} alphaTest={0.08} transparent opacity={0.95} depthWrite={false} blending={THREE.AdditiveBlending}/>
        </points>))}
    </>);
}
...
function SoundReceiveEffect({ requestId, mind, mentals, planeModelPath, onHighlightChange, onComplete, visualMode = 'sound', }: {
    requestId: number;
    mind: Mind;
    mentals: Mental[];
    planeModelPath: string;
    ...
    const mentalOriginalRef = useRef<Map<Mental, {
        pos: THREE.Vector3;
        frozen: boolean;
    }>>(new Map());
    ...
    useEffect(() => {
        ...
        const tryLoadEar = (index: number) => {
            if (cancelled || index >= earCandidates.length)
                return;
            loader.load(earCandidates[index], (gltf: {
                scene: THREE.Object3D;
            }) => {
                if (cancelled)
                    return;
                ...
                root.rotation.set(THREE.MathUtils.degToRad(earRotationXDeg), THREE.MathUtils.degToRad(earRotationYDeg), THREE.MathUtils.degToRad(earRotationZDeg));
    ...
    useEffect(() => {
        if (requestId <= 0 || requestId === prevRequestRef.current)
            return;
        prevRequestRef.current = requestId;
        runTokenRef.current += 1;
        const contact = mentals.find((m) => m.getName().trim().toLowerCase() === 'contact') ||
    ...
    const resolveMentalByName = useCallback((name: string): Mental | null => {
    ...
    const cleanupAndRestore = useCallback(() => {
        ...
        if (sender) {
            ...
            if (senderMesh?.parent)
                senderMesh.parent.remove(senderMesh);
            sender.dispose();
    ...
    const sendFromWorldToMental = useCallback(async (startWorld: THREE.Vector3, target: Mental): Promise<void> => {
        ...
        try {
            ...
            if (senderMeshNow?.parent)
                senderMeshNow.parent.remove(senderMeshNow);
            sender.dispose();
            if (senderMentalRef.current === sender)
                senderMentalRef.current = null;
        }
    }, [gl, isSceneMode, planeModelPath]);
    const sendFromWorldToWorld = useCallback(async (startWorld: THREE.Vector3, endWorld: THREE.Vector3): Promise<void> => {
        ...
        try {
            ...
            if (senderMesh.parent)
                senderMesh.parent.remove(senderMesh);
            if (receiverMesh.parent)
                receiverMesh.parent.remove(receiverMesh);
            sender.dispose();
            receiver.dispose();
        }
    }, [gl, isSceneMode, mind, planeModelPath]);
    const sendMentalToMental = useCallback(async (sender: Mental, receiver: Mental): Promise<void> => {
        await sender.sendDataTo(gl, receiver, {
            planeModelPath,
            durationMs: 980,
            arcHeight: 0.12,
    ...
    useFrame(() => {
        ...
        const radius = Math.max(1.1, mind.scale);
        if (isSceneMode) {
            earWorld.set(mind.position.x, mind.position.y + 0.24, mind.position.z + radius + 0.12);
        }
        else {
            earWorld.set(mind.position.x + radius + earSideGap, mind.position.y + earHeightOffset, mind.position.z + earDepthOffset);
        }
    });
    useEffect(() => {
        return () => {
    ...
    return (<>
      {earVisible && (<group position={[earWorld.x, earWorld.y, earWorld.z]}>
          {!isSceneMode && earModelRef.current ? (<primitive object={earModelRef.current}/>) : (isSceneMode ? (<>
...
function MindZoneBoundaries({ mind }: {
    mind: Mind;
}) {
    const mindRadius = mind.getRadius();
    ...
    const neutralBoundaryY = -0.3;
    const horizontalCircleRadius = Math.sqrt(Math.max(0, localRadius * localRadius - neutralBoundaryY * neutralBoundaryY));
    return (<group position={[mindPosition.x, mindPosition.y, mindPosition.z]}>
      
      <mesh position={[0, 0, 0]} rotation={[0, 0, Math.PI / 2]}>
        <planeGeometry args={[localRadius * 2.5, localRadius * 2.5]}/>
        <meshBasicMaterial color={0x00ff00} transparent opacity={0.3} side={THREE.DoubleSide}/>
      </mesh>
      
      <mesh position={[0, 0, 0]} rotation={[0, 0, Math.PI / 2]}>
        <planeGeometry args={[localRadius * 2.5, localRadius * 2.5]}/>
        <meshBasicMaterial color={0x00ff00} transparent opacity={0.8} side={THREE.DoubleSide} wireframe/>
      </mesh>
      ...
      <mesh position={[0, neutralBoundaryY * mindScale, 0]} rotation={[-Math.PI / 2, 0, 0]}>
        <planeGeometry args={[localRadius * 2.5, localRadius * 2.5]}/>
        <meshBasicMaterial color={0xff0000} transparent opacity={0.8} side={THREE.DoubleSide} wireframe/>
      </mesh>
      ...
      {horizontalCircleRadius > 0 && (<lineSegments position={[0, neutralBoundaryY * mindScale, 0]} rotation={[Math.PI / 2, 0, 0]}>
          <edgesGeometry args={[new THREE.CircleGeometry(horizontalCircleRadius, 64)]}/>
          <lineBasicMaterial color={0xff0000} linewidth={3}/>
        </lineSegments>)}
    </group>);
}
function PanelPositionSync({ focusTargetRef, selectedMentalName, onUpdate, }: {
    focusTargetRef: React.MutableRefObject<THREE.Vector3 | null>;
    selectedMentalName: string | null;
    onUpdate: ((pos: {
        x: number;
...
function XROccludedConnector({ focusTargetRef, selectedMentalName, enabled, panelWorldAnchorRef, }: {
    focusTargetRef: React.MutableRefObject<THREE.Vector3 | null>;
    selectedMentalName: string | null;
    enabled: boolean;
...
function TimelineCanvas({ stops, selectedIndex, onSelect, colors, t3HappySelected, onT3HappyChange, t5SelectedId, onT5Change, onSenseSelect, hasContactMental, personType, onPersonTypeChange, vithiCurrentEvent, timelineMode, onTimelineModeChange, slideshowPaused, onSlideshowPausedChange, vithiStageData, }: {
    stops: Array<{
        label: string;
        description: string;
    }>;
    ...
    const selectFromClientY = useCallback((clientY: number) => {
    ...
    return (<div style={{ display: 'flex', alignItems: 'center', gap: detailPanelOpen ? 18 : 10 }}>
      {detailPanelOpen && (<div style={{
                width: 360,
                maxHeight: 520,
                overflowY: 'auto',
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
            ...
            <button type="button" onClick={() => setDetailPanelOpen(false)} style={{
                padding: '5px 10px',
          ...
          {vithiStageData && vithiStageData.has(selectedIndex) && ((() => {
                const stage = vithiStageData.get(selectedIndex)!;
                return stage.blocked ? (<p style={{ margin: '8px 0 0', fontSize: 12, color: '#6b7280', fontStyle: 'italic' }}>No citta at this stage</p>) : (<div style={{ margin: '8px 0 0', fontSize: 12, color: '#a5b4fc' }}>
                  {stage.mental_details.length} cetasika(s): {stage.mental_details.map(d => d.name).join(', ')}
                </div>);
            })())}

          {timelineMode === 'slideshow' && selectedIndex > 0 && (<div style={{ marginTop: 10, display: 'flex', alignItems: 'center', gap: 8 }}>
              <button type="button" onClick={() => onSlideshowPausedChange?.(!slideshowPaused)} style={{
                    padding: '4px 12px',
                    borderRadius: 6,
                    border: '1px solid rgba(148,163,184,0.4)',
          ...
          {selectedIndex === 0 && (<div style={{ marginTop: 10, display: 'flex', alignItems: 'center', gap: 8 }}>
              <label style={{ fontSize: 11, color: '#94a3b8', fontWeight: 600 }}>Person type</label>
              <select value={personType ?? 'puthujjana'} onChange={(e) => onPersonTypeChange?.(e.target.value as PersonType)} style={{
                    fontSize: 12,
                    padding: '4px 8px',
                    borderRadius: 8,
                    border: '1px solid rgba(148, 163, 184, 0.4)',
                ...
                {PERSON_TYPES.map((pt) => (<option key={pt} value={pt}>{pt}</option>))}
              </select>
            </div>)}
          {selectedIndex === 0 && (<div style={{ marginTop: 10, display: 'flex', alignItems: 'center', gap: 8 }}>
              <label style={{ fontSize: 11, color: '#94a3b8', fontWeight: 600 }}>Timeline mode</label>
              <div style={{ display: 'flex', gap: 4 }}>
                {(['manual', 'slideshow'] as const).map((m) => (<button key={m} type="button" onClick={() => onTimelineModeChange?.(m)} style={{
                        padding: '3px 10px',
                        borderRadius: 6,
          ...
          {selectedIndex === 0 && !hasContactMental && (<p style={{ margin: '10px 0 0', fontSize: 12, color: '#f87171' }}>
              Create Contact (Phassa) mental via code runner to unlock sense doors
            </p>)}
          {selectedIndex === 0 && hasContactMental && (<div style={{ marginTop: 12 }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
                {T0_SENSE_OPTIONS.map((opt) => {
                    ...
                    return (<button key={opt.id} type="button" onClick={() => setSelectedSense(isSelected ? '' : opt.id)} style={{
                            display: 'flex',
                            alignItems: 'center',
                            gap: 6,
                            padding: '6px 10px',
              ...
              {selectedSense && SENSE_VARIANTS[selectedSense] && (<div style={{ marginTop: 10 }}>
                  <div style={{ fontSize: 11, color: '#94a3b8', marginBottom: 6, fontWeight: 600 }}>
                    What kind?
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                    {SENSE_VARIANTS[selectedSense].map((v) => (<button key={v.id} type="button" onClick={() => onSenseSelect?.(selectedSense, v.params)} style={{
                            display: 'flex',
                            alignItems: 'center',
                            gap: 8,
                            padding: '7px 12px',
          ...
          {selectedIndex === 3 && (<div style={{ marginTop: 12 }}>
              <div style={{ fontSize: 11, color: '#94a3b8', marginBottom: 8, fontWeight: 600 }}>
                Add option
              </div>
              <button type="button" onClick={() => onT3HappyChange?.(!t3HappySelected)} style={{
                    display: 'flex',
                    alignItems: 'center',
                    gap: 6,
                    padding: '7px 12px',
          ...
          {selectedIndex === 5 && (<div style={{ marginTop: 12 }}>
              <div style={{ fontSize: 11, color: '#94a3b8', marginBottom: 8, fontWeight: 600 }}>
                Choose mental factors (cetasikas)
              </div>
              <select value={t5SelectedId ?? ''} onChange={(e) => {
                ...
                {T5_CATEGORIES.map((cat) => (<optgroup key={cat.label} label={cat.label}>
                    {cat.optionIds.map((id) => {
                        const opt = T5_MENTAL_OPTIONS.find((o) => o.id === id);
                        return opt ? (<option key={opt.id} value={opt.id}>
                          {opt.label}
                        </option>) : null;
                    })}
                  </optgroup>))}
          ...
          {showDetail && (() => {
                ...
                {explanation ? (<p style={{ fontSize: 12, lineHeight: 1.6, color: '#c4b5fd' }}>
                    {explanation}
                  </p>) : (<p style={{ fontSize: 13, lineHeight: 1.5, color: '#9ca3af' }}>
                    {stops[selectedIndex].description}
                  </p>)}
                {stageInfo && !stageInfo.blocked && stageInfo.mental_details.length > 0 && (<div style={{ marginTop: 8, fontSize: 11, color: '#94a3b8' }}>
      ...
      <div style={{
        ...
        <div ref={railRef} onPointerDown={startDrag} style={{
            position: 'relative',
            height: '100%',
            width: 12,
            borderRadius: 999,
          ...
          {!detailPanelOpen && (<button type="button" onPointerDown={(e) => {
                e.stopPropagation();
            }} onClick={(e) => {
                e.stopPropagation();
          ...
          {stops.map((stop, i) => {
            ...
            return (<button key={stop.label} type="button" onClick={() => {
                    onSelect(i);
                    if (maxIndex > 0)
                        setPinRatio(i / maxIndex);
                }} title={stop.label} style={{
                    position: 'absolute',
                    top: `${topPct}%`,
                    left: '50%',
...
function XRTimelineToggle({ enabled, onToggle, }: {
    enabled: boolean;
    onToggle: () => void;
}) {
    const { gl } = useThree();
    ...
    useFrame((_state, delta) => {
        ...
        for (const inputSource of session.inputSources) {
            ...
            if (!gamepad || !gamepad.buttons?.length)
                continue;
            bPressed = Boolean(gamepad.buttons[5]?.pressed ||
...
function XRTimelinePanel({
  ...
  subStepIndex?: number
  onSubStepChange?: (idx: number) => void
function XRTimelinePanel({ stops, selectedIndex, onSelect, panelOpen, onOpenPanel, onClosePanel, t3HappySelected, onT3HappyChange, t5SelectedId, onT5Change, onSenseSelect, hasContactMental, personType, onPersonTypeChange, timelineMode, onTimelineModeChange, slideshowPaused, onSlideshowPausedChange, vithiCurrentEvent, vithiStageData, }: {
    stops: Array<{
        label: string;
        description: string;
    }>;
  ...
  return (
    ...
    useEffect(() => {
        ...
        const handleXrSelect = (event: Event) => {
            ...
            if (selectedIndex === 5) {
                const flatOptions = T5_CATEGORIES.flatMap((cat) => cat.optionIds
                    .map((id) => T5_MENTAL_OPTIONS.find((opt) => opt.id === id))
        ...
        if (gl.xr.isPresenting) {
            ...
            for (const controller of controllers) {
                ...
                if (openPanelButtonRef.current) {
                    nextOpenHovered = hitInside(openPanelButtonRef.current);
                    if (nextOpenHovered)
                        nextHoveredAction = 'open-timeline';
                }
                if (closePanelButtonRef.current) {
                    nextCloseHovered = hitInside(closePanelButtonRef.current);
                    if (nextCloseHovered)
                        nextHoveredAction = 'close-timeline';
                }
                if (!nextHoveredAction && hitInside(explainButtonRef.current))
                    nextHoveredAction = 'explain-detail';
                if (!nextHoveredAction && hitInside(happyButtonRef.current))
                    nextHoveredAction = 'happy-toggle';
                if (!nextHoveredAction && hitInside(t5PrevButtonRef.current))
                    nextHoveredAction = 't5-prev';
                if (!nextHoveredAction && hitInside(t5NextButtonRef.current))
                    nextHoveredAction = 't5-next';
                if (!nextHoveredAction && hitInside(t5ClearButtonRef.current))
                    nextHoveredAction = 't5-clear';
                if (!nextHoveredAction && hitInside(personTypePrevButtonRef.current))
                    nextHoveredAction = 'person-prev';
                if (!nextHoveredAction && hitInside(personTypeNextButtonRef.current))
                    nextHoveredAction = 'person-next';
                if (!nextHoveredAction && hitInside(modeManualButtonRef.current))
                    nextHoveredAction = 'mode-manual';
                if (!nextHoveredAction && hitInside(modeSlideshowButtonRef.current))
                    nextHoveredAction = 'mode-slideshow';
                if (!nextHoveredAction && hitInside(slideshowPauseButtonRef.current))
                    nextHoveredAction = 'slideshow-pause';
                if (!nextHoveredAction) {
                    for (const opt of T0_SENSE_OPTIONS) {
                        if (hitInside(senseButtonRefs.current[opt.id])) {
                            nextHoveredAction = `sense-${opt.id}`;
                ...
                if (nextHovered !== null || nextOpenHovered || nextCloseHovered || nextHoveredAction)
                    break;
            }
        }
        if (hoveredIndexRef.current !== nextHovered) {
    ...
    if (selectedIndex === 3)
        contentBottomY = -0.2;
    if (selectedIndex === 5)
        contentBottomY = -0.265;
    if (vithiCurrentEvent)
        contentBottomY = Math.min(contentBottomY, -0.32);
    const explainButtonY = Math.max(-0.52, contentBottomY - 0.09);
    const panelBottomY = explainButtonY - 0.075;
    const panelHeight = Math.max(0.84, panelTopY - panelBottomY);
    const innerPanelHeight = Math.max(0.2, panelHeight - 0.004);
    ...
    return (<group ref={groupRef}>
      ...
      {stops.map((stop, i) => {
            ...
            return (<group key={stop.label}>
            <mesh ref={(node: THREE.Mesh | null) => {
      ...
      {panelOpen && (<group>
          ...
          <Text position={[-0.88, headerDescTopY, 0.005]} anchorX="left" anchorY="top" fontSize={0.034} maxWidth={0.78} lineHeight={1.2} color="#cbd5e1">
          ...
          {vithiStageData && vithiStageData.has(selectedIndex) && ((() => {
                ...
                const stageText = stage.blocked
                    ? 'No citta at this stage'
                    : `${stage.mental_details.length} cetasika(s): ${stage.mental_details.map((d) => d.name).join(', ')}`;
                return (<Text position={[-0.88, stageInfoY, 0.006]} anchorX="left" anchorY="middle" fontSize={0.018} color={stage.blocked ? '#6b7280' : '#a5b4fc'} maxWidth={0.78}>
                  {stageText}
                </Text>);
            })())}
          ...
          {selectedIndex === 0 && !hasContactMental && (<Text position={[-0.88, senseHeaderY, 0.006]} anchorX="left" anchorY="middle" fontSize={0.02} color="#f87171" maxWidth={0.78}>
              Create Contact (Phassa) mental via code runner to unlock sense doors
            </Text>)}
          {selectedIndex === 0 && hasContactMental && (<>
              ...
              {T0_SENSE_OPTIONS.map((opt, idx) => {
                    ...
                    <Text position={[x - 0.097, y, 0.007]} anchorX="left" anchorY="middle" fontSize={0.024} color={isSelected || isActionHovered(`sense-${opt.id}`) ? '#f8fafc' : '#e2e8f0'}>
                      {`${opt.icon} ${opt.label}`}
                    </Text>
                  </group>);
                })}
              {selectedVariants.length > 0 && (<>
                  ...
                  {selectedVariants.slice(0, 3).map((variant, idx) => {
                        ...
                        <Text position={[-0.86, y, 0.007]} anchorX="left" anchorY="middle" fontSize={0.022} color={isActionHovered(`variant-${variant.id}`) ? '#ffffff' : '#ecfeff'} maxWidth={0.74}>
          ...
          {selectedIndex === 3 && (<>
              ...
              <Text position={[-0.84, -0.16, 0.007]} anchorX="left" anchorY="middle" fontSize={0.024} color={isActionHovered('happy-toggle') ? '#f8fafc' : '#e2e8f0'}>
                😊 Happy
              </Text>
            </>)}
          ...
          {selectedIndex === 5 && (<>
              ...
              <Text position={[-0.72, -0.225, 0.007]} anchorX="center" anchorY="middle" fontSize={0.022} color={isActionHovered('t5-prev') ? '#ffffff' : '#f8fafc'}>
                Prev
              </Text>
              ...
              <Text position={[-0.48, -0.225, 0.007]} anchorX="center" anchorY="middle" fontSize={0.022} color={isActionHovered('t5-next') ? '#ffffff' : '#f8fafc'}>
                Next
              </Text>
              ...
              <Text position={[-0.24, -0.225, 0.007]} anchorX="center" anchorY="middle" fontSize={0.022} color={isActionHovered('t5-clear') ? '#ffffff' : '#fee2e2'}>
                Clear
              </Text>
            </>)}
          ...
          {showDetail && (() => {
                ...
                const text = explanation
                    ...
                    : activeStop.description;
                return (<Text position={[-0.52, explainButtonY, 0.007]} anchorX="left" anchorY="middle" fontSize={0.018} color="#c4b5fd" maxWidth={0.42}>
                {text}
              </Text>);
            })()}
...
function InspectOptionMenu({ selection, panelPosition, onClose, onViewDetail, onVoice, voiceLoading = false, onDragPositionChange, }: {
    ...
    const renderOption = (label: string, description: string, onClick: () => void, disabled = false) => (<button key={label} type="button" style={optionStyle} onClick={onClick} disabled={disabled} onMouseEnter={(e) => {
            ;
...
function ThreeScene({ mind, mentals, selected, profile, profileAttrs, selectedMentalName, inspectOpen, onSelectMental, onViewDetail, onBackFromDetail, onShowProfile, onCloseProfile, onCloseSelection, onVoiceSelection, onUpdatePanelPosition, sendMode, emojiMode, onSendSelection, soundReceiveRequestId, sceneReceiveRequestId, onSoundReceiveHighlightChange, onSceneReceiveHighlightChange, onSoundReceiveComplete, onSceneReceiveComplete, showHumanModel, humanShape, xrMode, defaultMindPosition, defaultMindScale, onRendererReady, searchHighlight, explainHighlight, timelineIndex, onTimelineSelect, t3HappySelected, onT3HappyChange, t5SelectedId, onT5Change, onSenseSelect, hasContactMental, vithiCurrentEvent, stops, personType, onPersonTypeChange, timelineMode, onTimelineModeChange, slideshowPaused, onSlideshowPausedChange, vithiStageData, xrTimelineOpen, xrTimelineDetailOpen, onToggleXrTimeline, onOpenXrTimelineDetail, onCloseXrTimelineDetail, }: {
    mind: Mind;
    mentals: Mental[];
    selected: InspectSelection | null;
    profile: InspectSelection | null;
    ...
    const effectiveHumanGroundY = useMemo(() => {
        if (!isXrActive)
            return DEFAULT_HUMAN_GROUND_Y;
        return DEFAULT_HUMAN_GROUND_Y + XR_MIND_HEIGHT_OFFSET;
    }, [isXrActive]);
    const xrInitialOffset = useMemo<[
        number,
        number,
        number
    ]>(() => {
    ...
    const handleSelectMental = useCallback((info: InspectSelection) => {
        if (isXrActive && xrTimelineOpen)
            return;
        onSelectMental(info);
    }, [isXrActive, onSelectMental, xrTimelineOpen]);
    const outlineSelection = (explainHighlight?.length ?? 0) > 0
        ? explainHighlight!
    ...
    return (<Canvas camera={{ position: [0, 0, 4], fov: 75 }} shadows={!isArMode} gl={{ antialias: true, toneMappingExposure: 0.95, alpha: isArMode }} onCreated={({ gl }) => {
            if (isArMode)
      ...
      <OrbitControls ref={controlsRef} enabled={!isXrActive} enableDamping={!selectedMentalName} dampingFactor={selectedMentalName ? 0 : 0.05} enableZoom enablePan={false} enableRotate={!selectedMentalName && !isXrActive} minDistance={0.35} maxDistance={24} maxPolarAngle={Math.PI / 2 - 0.02} target={[mind.position.x, mind.position.y, mind.position.z]}/>
      ...
      <SoundReceiveEffect requestId={soundReceiveRequestId} mind={mind} mentals={mentals} planeModelPath={paperPlaneModel} onHighlightChange={onSoundReceiveHighlightChange} onComplete={onSoundReceiveComplete}/>
      <SoundReceiveEffect requestId={sceneReceiveRequestId} mind={mind} mentals={mentals} planeModelPath={paperPlaneModel} onHighlightChange={onSceneReceiveHighlightChange} onComplete={onSceneReceiveComplete} visualMode="scene"/>
      {showHumanInScene && (<React.Suspense fallback={null}>
          <HumanBody mind={mind} controlsRef={controlsRef} selectedShape={humanShape} groundY={effectiveHumanGroundY}/>
        </React.Suspense>)}
      <MindSphere mind={mind}/>
      {showMentalsLayer && (<MentalsLayer mind={mind} mentals={mentals} selectedMentalName={selectedMentalName} onSelectMental={handleSelectMental} controlsRef={controlsRef} focusTargetRef={focusTargetRef} planeModelPath={paperPlaneModel} sendMode={sendMode} emojiMode={emojiMode} blockXrMentalPick={isXrActive && xrTimelineOpen} onSendSelection={onSendSelection} onHoverSelection={setHoverSelection} onSendMeshSelection={setSendMeshSelection}/>)}
      {isXrActive && selected && !profile && (<XRInspectPanel selection={selected} inspectOpen={inspectOpen} onViewDetail={onViewDetail} onBack={onBackFromDetail} onShowProfile={onShowProfile} onVoice={onVoiceSelection} onClose={onCloseSelection} panelWorldAnchorRef={xrPanelWorldAnchorRef}/>)}
      {isXrActive && profile && (<XRProfilePanel profile={profile} attrs={profileAttrs} onBack={onCloseProfile} panelWorldAnchorRef={xrPanelWorldAnchorRef}/>)}
      {isXrActive && (<XRTimelineToggle enabled={isXrActive} onToggle={onToggleXrTimeline}/>)}
      {isXrActive && xrTimelineOpen && (<XRTimelinePanel stops={stops} selectedIndex={timelineIndex} onSelect={onTimelineSelect} panelOpen={xrTimelineDetailOpen} onOpenPanel={onOpenXrTimelineDetail} onClosePanel={onCloseXrTimelineDetail} t3HappySelected={t3HappySelected} onT3HappyChange={onT3HappyChange} t5SelectedId={t5SelectedId} onT5Change={onT5Change} onSenseSelect={onSenseSelect} hasContactMental={hasContactMental} personType={personType} onPersonTypeChange={onPersonTypeChange} timelineMode={timelineMode} onTimelineModeChange={onTimelineModeChange} slideshowPaused={slideshowPaused} onSlideshowPausedChange={onSlideshowPausedChange} vithiCurrentEvent={vithiCurrentEvent} vithiStageData={vithiStageData}/>)}
      <XROccludedConnector focusTargetRef={focusTargetRef} selectedMentalName={selectedMentalName} enabled={isXrActive} panelWorldAnchorRef={xrPanelWorldAnchorRef}/>
      <PanelPositionSync focusTargetRef={focusTargetRef} selectedMentalName={selectedMentalName} onUpdate={onUpdatePanelPosition}/>
      {showMentalsLayer && !isXrActive && (<EffectComposer multisampling={2} autoClear={false}>
          <Outline selection={outlineSelection} blendFunction={BlendFunction.ALPHA} visibleEdgeColor={0xffffff} hiddenEdgeColor={0x190a05} edgeStrength={30} resolutionScale={1} xRay/>
        </EffectComposer>)}
    </Canvas>);
}
export function Simulation(): React.ReactElement {
  ...
  const connectorAnimatedEnd =
    ...
    const [profile, setProfile] = useState<InspectSelection | null>(null);
    const [profileAttrs, setProfileAttrs] = useState<Array<{
        key: string;
        value: string;
    }>>([]);
    const [profileMarkers, setProfileMarkers] = useState<Array<{
        key: string;
        value: string;
        position: {
            x: number;
    ...
    const [activeVariants, setActiveVariants] = useState<Set<string>>(new Set());
    const vithiTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
    const [codeRunnerPos, setCodeRunnerPos] = useState<{
        x: number;
        y: number;
    }>({ x: 38, y: 104 });
    const codeRunnerDragRef = useRef<{
        active: boolean;
        dx: number;
        dy: number;
    ...
    const [isMindExplaining, setIsMindExplaining] = useState(false);
    const [isInspectVoicePlaying, setIsInspectVoicePlaying] = useState(false);
    const [explainOverlay, setExplainOverlay] = useState<{
        name: string;
        detail: string;
    ...
    const { activeXrMode, vrSupport, arSupport, vrMessage, arMessage, vrButtonDisabled, vrButtonTitle, arButtonDisabled, arButtonTitle, handleToggleVr, handleToggleAr, } = useXRSession({
        renderer,
        overlayRoot: overlayRootRef.current,
    });
    const isXrActive = activeXrMode !== null;
    ...
    const [mentals, setMentals] = useState<Mental[]>(() => getMentalVariantsForTimelineStop(0, false, null).map((variant) => getOrCreateMental(variant)));
    useEffect(() => {
        if (vithiStageData && vithiStageData.size > 0) {
            const allVariants = new Set<MentalVariant>();
            const currentVariants = new Set<string>();
            for (let t = 0; t <= timelineIndex; t++) {
                ...
                stageVars.forEach((v) => allVariants.add(v));
                if (t === timelineIndex)
                    stageVars.forEach((v) => currentVariants.add(v));
            }
            setActiveVariants(currentVariants);
            setMentals([...allVariants].map((v) => getOrCreateMental(v)));
            return;
    ...
    const hasContactMental = useMemo(() => allMentals.some((m) => m instanceof ContactMental), [allMentals]);
    useEffect(() => {
        mind.setLabelDepthOcclusion(isXrActive);
        allMentals.forEach((mental) => {
            mental.setLabelDepthOcclusion(isXrActive);
    ...
    useEffect(() => {
        ...
        if (signature === timelineSignatureRef.current)
            return;
        timelineSignatureRef.current = signature;
        if (!scriptResultActive && scriptMentals.length === 0 && scriptMatchedDefaultMentals.length === 0)
            return;
        scriptMentalMapRef.current.forEach((mental) => mental.dispose());
        scriptMentalMapRef.current.clear();
        setScriptMentals([]);
    ...
    const parseNumberList = useCallback((value: string): [
        number,
        number,
        number
    ] | null => {
    ...
    const formatVec = useCallback((vec: [
        number,
        number,
        number
    ]): string => {
    ...
    const handleCodeRunnerTimelinePreset = useCallback((presetId: string) => {
        setCodeRunnerTimelinePreset(presetId);
        setCodeRunnerDirty(false);
        setCodeRunnerErrorLine(null);
        if (presetId === '__current__') {
    ...
    useEffect(() => {
        if (codeRunnerDirty)
            return;
        let preset = codeRunnerTimelinePreset;
        const presetOkForStep = preset === '__current__' ||
            preset === '__custom__' ||
            (timelineIndex === 3 && (preset === 't3-unhappy' || preset === 't3-happy')) ||
            (timelineIndex === 5 && preset.startsWith('t5-'));
        if (!presetOkForStep) {
    ...
    useEffect(() => {
        if (timelineMode !== 'slideshow' || slideshowPaused)
            return;
        const maxIdx = TIMELINE_STOPS.length - 1;
    ...
    const handleRunCodeRunner = useCallback(async () => {
        ...
        mentals.forEach((mental) => {
            const key = normalizeMentalName(mental.getName());
            if (!key || defaultMentalsByName.has(key))
                return;
            defaultMentalsByName.set(key, mental);
        });
        scriptMentalMapRef.current.forEach((mental) => mental.dispose());
        ...
        actions.forEach((action) => {
            if (action.type === 'create_mind') {
                ...
                if (!linkedMentalVars.has(action.variable))
                    return;
                const targetName = plannedMentalNameByVar.get(action.variable) || action.data.name || '';
                const maybeName = normalizeMentalName(targetName);
                const matchedDefault = maybeName.length > 0 ? defaultMentalsByName.get(maybeName) ?? null : null;
                const canReuseDefault = Boolean(matchedDefault && !usedDefaultMentals.has(matchedDefault));
                const mental = canReuseDefault && matchedDefault
                    ? matchedDefault
                    : new Mental({
                        name: action.data.name || 'Mental Sphere',
                        detail: '',
                ...
                if (action.attribute === 'name')
                    mind.setName(action.value);
                else if (action.attribute === 'color')
                    mind.setColor(action.value);
                else if (action.attribute === 'scale') {
                    const scale = Number.parseFloat(action.value);
                ...
                if (!mental)
                    return;
                if (action.attribute === 'name')
                    mental.setName(action.value);
                else if (action.attribute === 'color')
                    mental.setColor(action.value);
                else if (action.attribute === 'scale') {
                    const scale = Number.parseFloat(action.value);
                    if (!Number.isNaN(scale))
                        mental.setScale(scale);
                }
                else if (action.attribute === 'position') {
                    const vec = parseNumberList(action.value);
                    if (vec)
                        mental.setPosition(vec);
                }
                else if (action.attribute === 'detail') {
                    mental.setDetail(action.value);
                }
        ...
        try {
            ...
            const data: {
                summary?: {
                    minds_created?: number;
                    mentals_created?: number;
                };
                created_minds?: Array<{
                    id: number;
                }>;
    ...
    const handleSenseSelect = useCallback((senseId: string, variantParams: VithiParams) => {
        ...
        fetch(`${API_BASE}/api/vithi/pancadvara`, {
            ...
            .then((data: {
            events?: VithiEvent[];
            updated_experience_weight?: Record<string, number>;
        }) => {
            if (data.events)
                setVithiQueue(data.events);
            if (data.updated_experience_weight && sannaMental) {
                sannaMental.updateExperienceWeightFromResponse(data.updated_experience_weight);
            }
            if (data.events && data.events.length > 0) {
                ...
                for (let tIdx = 0; tIdx < VITHI_STAGE_ORDER.length; tIdx++) {
                    ...
                    let desc = '';
                    for (const ev of eventsForStage) {
                        if (ev.mind_id && !firstMindId)
                            firstMindId = ev.mind_id;
                        if (ev.mind_id_range) {
                            if (!rangeMindIds)
                                rangeMindIds = [...ev.mind_id_range];
                            else {
                                const lo = Math.min(rangeMindIds[0], ev.mind_id_range[0]);
                                const hi = Math.max(rangeMindIds[rangeMindIds.length - 1], ev.mind_id_range[ev.mind_id_range.length - 1]);
                                rangeMindIds = [lo, hi];
                        ...
                        if (ev.mind_name && !mindName)
                            mindName = ev.mind_name;
                        if (ev.description)
                            desc = ev.description;
                        for (const mid of (ev.mental_ids ?? []))
                            allMentalIds.add(mid);
                        for (const d of (ev.mental_details ?? []))
    ...
    const handleCodeRunnerPointerDown = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
        if ((e.target as HTMLElement).closest('[data-cr-drag-handle]') === null)
            return;
        codeRunnerDragRef.current.active = true;
        codeRunnerDragRef.current.dx = e.clientX - codeRunnerPos.x;
        codeRunnerDragRef.current.dy = e.clientY - codeRunnerPos.y;
        const handleMove = (ev: PointerEvent) => {
            if (!codeRunnerDragRef.current.active)
                return;
            setCodeRunnerPos({
                x: Math.max(8, ev.clientX - codeRunnerDragRef.current.dx),
                y: Math.max(8, ev.clientY - codeRunnerDragRef.current.dy),
    ...
    const getOverlayPoint = useCallback((point: {
        x: number;
        y: number;
    } | null): {
        x: number;
        ...
        if (!point || !overlayRootRef.current || typeof window === 'undefined')
            return null;
        const rect = overlayRootRef.current.getBoundingClientRect();
        return {
            x: point.x - (rect.left + window.scrollX),
    ...
    return (<main className="page simulation-page">
      <div ref={overlayRootRef} className="simulation-full" style={{ position: 'relative' }}>
        {showMenuConnector && menuConnectorStart && connectorAnimatedEnd && (<svg style={{
                position: 'absolute',
                inset: 0,
                width: '100%',
                height: '100%',
            ...
            <line x1={menuConnectorStart.x} y1={menuConnectorStart.y} x2={connectorAnimatedEnd.x} y2={connectorAnimatedEnd.y} stroke="url(#inspect-menu-connector-gradient)" strokeWidth={2.8} strokeLinecap="round"/>
          </svg>)}
        <div className="send-toolbar" style={{
            position: 'absolute',
            top: 12,
            left: 12,
            zIndex: 10,
        ...
        }} disabled={soundReceiveActive || sceneReceiveActive} style={{
            padding: '6px 10px',
            borderRadius: 6,
            border: 'none',
          ...
          <button type="button" onClick={handleExplainMind} disabled={isMindExplaining} style={{
            padding: '6px 10px',
            borderRadius: 6,
            border: 'none',
          ...
          {showHumanModel && (<select value={humanShape} onChange={(e) => setHumanShape(e.target.value as MorphShapeKey)} style={{
                padding: '6px 8px',
                borderRadius: 6,
          ...
          <button type="button" onClick={() => setSearchOpen((prev) => !prev)} style={{
            padding: '6px 10px',
            borderRadius: 6,
            border: 'none',
          ...
          {searchOpen && (<>
              <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && setSearchOpen(false)} placeholder="Search mental sphere..." style={{
                padding: '6px 10px',
                borderRadius: 6,
                border: '1px solid rgba(255,255,255,0.3)',
              ...
              <button type="button" onClick={() => setSearchOpen(false)} style={{ padding: '6px 10px', borderRadius: 6, border: 'none', background: 'rgba(255,255,255,0.2)', color: 'white', cursor: 'pointer', fontSize: 12 }}>
                Close
              </button>
            </>)}
          <button type="button" onClick={() => {
          ...
          <button type="button" onClick={handleToggleVr} disabled={vrButtonDisabled} title={vrButtonTitle} style={{
            padding: '6px 10px',
            borderRadius: 6,
            border: 'none',
          ...
          <button type="button" onClick={handleToggleAr} disabled={arButtonDisabled} title={arButtonTitle} style={{
            padding: '6px 10px',
            borderRadius: 6,
            border: 'none',
        ...
        {isMindExplaining && explainOverlay && (<div style={{
                position: 'absolute',
                left: 26,
                top: 68,
                zIndex: 14,
        ...
        {codeRunnerOpen && (<div onPointerDown={handleCodeRunnerPointerDown} style={{
                position: 'absolute',
            ...
            <div data-cr-drag-handle style={{
              ...
              <button type="button" onClick={() => setCodeRunnerOpen(false)} style={{
                border: 'none',
                borderRadius: 6,
                padding: '2px 8px',
            ...
            <div style={{ padding: 10, display: 'grid', gap: 8 }}>
              ...
              {showTimelineScriptPresetDropdown && (<label style={{ display: 'grid', gap: 4, fontSize: 12, opacity: 0.9 }}>
                  <span>
                    {timelineIndex === 3
                  ...
                  <select value={codeRunnerTimelinePreset === '__custom__' ||
                    codeRunnerTimelinePreset === '__current__' ||
                    timelineScriptPresetsForStep.some((p) => p.id === codeRunnerTimelinePreset)
                    ? codeRunnerTimelinePreset
                    : '__current__'} onChange={(e) => handleCodeRunnerTimelinePreset(e.target.value)} style={{
                    width: '100%',
                    padding: '6px 8px',
                    borderRadius: 6,
                    border: '1px solid rgba(148,163,184,0.4)',
                    ...
                    {codeRunnerTimelinePreset === '__custom__' ? (<option value="__custom__">Custom (edited in editor)</option>) : null}
                    {timelineIndex === 3
                    ? timelineScriptPresetsForStep.map((p) => (<option key={p.id} value={p.id}>
                            {p.label}
                          </option>))
                    : timelineIndex === 5
                        ? (<>
                              ...
                              {T5_CATEGORIES.map((cat) => (<optgroup key={cat.label} label={cat.label}>
                                  {cat.optionIds.map((oid) => {
                                    ...
                                    if (!pick)
                                        return null;
                                    const optLabel = T5_MENTAL_OPTIONS.find((o) => o.id === oid)?.label ?? pick.label;
                                    return (<option key={pick.id} value={pick.id}>
                                        {optLabel}
                                      </option>);
                                })}
                                </optgroup>))}
              ...
              <button type="button" onClick={handleDownloadAllTimelineScripts} style={{
                padding: '6px 10px',
                borderRadius: 6,
              ...
              <textarea value={codeRunnerCode} onChange={(e) => {
                ...
                color: '#e2e8f0',
                border: codeRunnerErrorLine !== null
                ...
                fontSize: 12,
                lineHeight: 1.45,
                boxShadow: codeRunnerErrorLine !== null
                    ? 'inset 0 -2px 0 rgba(239,68,68,0.85)'
                    : 'none',
            }}/>
              {codeRunnerErrorLine !== null && (<div style={{
                    fontSize: 12,
                    color: '#fca5a5',
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                <button type="button" onClick={handleRunCodeRunner} style={{
                padding: '6px 10px',
                borderRadius: 6,
                border: 'none',
                background: '#3b82f6',
                ...
                <button type="button" onClick={handleClearCodeRunnerMentals} style={{
                padding: '6px 10px',
                borderRadius: 6,
                border: 'none',
                background: '#b91c1c',
        ...
        >
          ...
          {!isXrActive && (<TimelineCanvas stops={dynamicStops} selectedIndex={timelineIndex ?? 0} onSelect={setTimelineIndex} colors={TIMELINE_COLORS} t3HappySelected={t3HappySelected} onT3HappyChange={setT3HappySelected} t5SelectedId={t5SelectedId} onT5Change={setT5SelectedId} onSenseSelect={handleSenseSelect} hasContactMental={hasContactMental} personType={personType} onPersonTypeChange={setPersonType} vithiCurrentEvent={vithiCurrentEvent} timelineMode={timelineMode} onTimelineModeChange={setTimelineMode} slideshowPaused={slideshowPaused} onSlideshowPausedChange={setSlideshowPaused} vithiStageData={vithiStageData}/>)}
        </div>
        ...
        />
        {selected && !isXrActive && !inspectOpen && menuRevealReady && (<InspectOptionMenu selection={selected} panelPosition={panelPosition} onClose={handleClose} onViewDetail={handleViewDetail} onVoice={handleSpeakInspectSelection} voiceLoading={isInspectVoicePlaying} onDragPositionChange={setPanelPosition}/>)}
        {selected && !isXrActive && inspectOpen && (<InspectPanel selection={selected} panelPosition={panelPosition} onClose={handleClose} onShowProfile={handleShowProfile} onVoice={handleSpeakInspectSelection} voiceLoading={isInspectVoicePlaying} onDragPositionChange={setPanelPosition}/>)}
        {profile && !isXrActive && (<ProfilePanel profile={profile} attrs={profileAttrs} markers={profileMarkers} attrKey={attrKey} attrValue={attrValue} onAttrKeyChange={setAttrKey} onAttrValueChange={setAttrValue} onAddAttribute={handleAddAttribute} onClose={handleCloseProfile}/>)}
        <ThreeScene mind={mind} mentals={allMentals} selected={selected} profile={profile} profileAttrs={profileAttrs} selectedMentalName={selected?.name ?? null} inspectOpen={inspectOpen} onSelectMental={handleSelect} onViewDetail={handleViewDetail} onBackFromDetail={handleBackFromDetail} onShowProfile={handleShowProfile} onCloseProfile={handleCloseProfile} onCloseSelection={handleClose} onVoiceSelection={handleSpeakInspectSelection} onUpdatePanelPosition={inspectOpen ? undefined : setFocusScreenPosition} sendMode={sendMode} emojiMode={emojiMode} onSendSelection={setSendInfo} soundReceiveRequestId={soundReceiveRequestId} sceneReceiveRequestId={sceneReceiveRequestId} onSoundReceiveHighlightChange={setSoundReceiveHighlight} onSceneReceiveHighlightChange={setSoundReceiveHighlight} onSoundReceiveComplete={() => {
            setSoundReceiveActive(false);
            setSoundReceiveHighlight([]);
            setSendInfo({ sender: 'Sound', receiver: 'Contact', status: 'Delivered' });
        }} onSceneReceiveComplete={() => {
            ...
            setSoundReceiveHighlight([]);
            setSendInfo({ sender: 'Scene', receiver: 'Contact', status: 'Delivered' });
        }} showHumanModel={showHumanModel} humanShape={humanShape} xrMode={activeXrMode} defaultMindPosition={DEFAULT_MIND_POSITION} defaultMindScale={DEFAULT_MIND_SCALE} searchHighlight={searchHighlight} explainHighlight={soundReceiveHighlight.length > 0 ? soundReceiveHighlight : explainHighlight} stops={dynamicStops} timelineIndex={timelineIndex} onTimelineSelect={setTimelineIndex} t3HappySelected={t3HappySelected} onT3HappyChange={setT3HappySelected} t5SelectedId={t5SelectedId} onT5Change={setT5SelectedId} onSenseSelect={handleSenseSelect} hasContactMental={hasContactMental} personType={personType} onPersonTypeChange={setPersonType} timelineMode={timelineMode} onTimelineModeChange={setTimelineMode} slideshowPaused={slideshowPaused} onSlideshowPausedChange={setSlideshowPaused} vithiCurrentEvent={vithiCurrentEvent} vithiStageData={vithiStageData} xrTimelineOpen={xrTimelineOpen} xrTimelineDetailOpen={xrTimelineDetailOpen} onToggleXrTimeline={() => setXrTimelineOpen((prev) => {
            const next = !prev;
            if (!next)
                setXrTimelineDetailOpen(false);
            return next;
        })} onOpenXrTimelineDetail={() => setXrTimelineDetailOpen(true)} onCloseXrTimelineDetail={() => setXrTimelineDetailOpen(false)} onRendererReady={(gl) => {
            setRenderer(gl);
        }}/>
      </div>
    </main>);
