...
const RUPA_GROUP_META: Record<string, {
    titleEn: string;
    titleTh: string;
    color: string;
}> = {
...
const RUPA_SUBGROUP_META: Record<string, {
    titleEn: string;
    titleTh: string;
    color: string;
}> = {
...
function mentalSphereRingClass(mentalId: number): string {
    if (mentalId === 52)
        return 'mental-ring-thick';
    if ([13, 27, 47, 48, 49].includes(mentalId))
        return 'mental-ring';
    return '';
}
...
export function MindStudy(): React.ReactElement {
    ...
    useEffect(() => {
        ...
        Promise.all([
            ...
            for (const r of rupasRes.rupas ?? [])
                console.log('Rupa:', JSON.stringify(r));
            ...
            setAggregates(fetchedMinds.map((mind) => new DhammaObject({
                id: `mind-${mind.id}`,
                title: mind.name_en || mind.name || mind.thai,
                description: mind.description?.trim() ||
    ...
    const playPoolNarrative = useCallback(async () => {
        ...
        const playWithGoogleTTS = (text: string): Promise<void> => new Promise((resolve, reject) => {
            if (!apiKey) {
        ...
        const speakNext = async () => {
            ...
            if (line === DROPLET_LINE)
                scheduleDropletDrops();
            try {
    ...
    const mindDiagramGroups = useMemo(() => {
        ...
        const groupsFromApiRaw = staticMindGroups
            ...
            const isKamavacaraSobhanaGroup = /Kamavacara Sobhana|กามาวจรโสภณ/i.test(`${group.name} ${group.name_thai} ${group.name_en}`);
            return {
        ...
        const kamavacaraGroups = groupsFromApiRaw.filter((group) => /กามาวจร|Kāmāvacara|Kamavacara/i.test(`${group.title} ${group.subtitle}`));
        const groupsFromApi = kamavacaraGroups.length > 1
            ? (() => {
                const nonKamavacaraGroups = groupsFromApiRaw.filter((group) => !/กามาวจร|Kāmāvacara|Kamavacara/i.test(`${group.title} ${group.subtitle}`));
                ...
                const ahetukaIndex = nonKamavacaraGroups.findIndex((group) => /อเหตุก|Ahetuka/i.test(`${group.title} ${group.subtitle}`));
                if (ahetukaIndex >= 0) {
                    return [
        ...
        const groupsWithMahaggataSplit = groupsFromApi.flatMap((group) => {
            if (!/มหัคคต|Mahaggata/i.test(`${group.title} ${group.subtitle}`))
                return [group];
        ...
        if (groupsWithMahaggataSplit.length)
            return groupsWithMahaggataSplit;
        ...
        staticMinds.forEach((mind) => {
            ...
            if (!fallbackByCategory.has(key))
                fallbackByCategory.set(key, []);
    ...
    const staticMindByAggregateId = useMemo(() => new Map(staticMinds.map((mind) => [`mind-${mind.id}`, mind])), [staticMinds]);
    const rupaCategoryBlocks = useMemo(() => {
        ...
        return Array.from(rowsByGroup.entries())
            ...
            const sortedRows = rows.sort((a, b) => RUPA_SUBGROUP_ORDER.indexOf(a.subgroup) - RUPA_SUBGROUP_ORDER.indexOf(b.subgroup));
            return {
                id: `rupa-group-${group}`,
    ...
    const staticMentalById = useMemo(() => new Map(staticMentals.map((mental) => [mental.id, mental])), [staticMentals]);
    const openCetasikaFromStaticMental = useCallback((mental: StaticMental, options?: {
        closeMindModal?: boolean;
    }) => {
        setSelectedCetasika({
            id: `mental-${mental.id}`,
            pali: mental.pali,
    ...
    const mentalCategoryBlocks = useMemo(() => {
        ...
        const mapRows = (wantedIds: number[], rowColors: string[], anchorMap: Record<number, string>) => wantedIds
            .map((id, index) => {
        ...
        const neutralRows = mapRows([1, 2], neutralRowColors, { 1: 'cetasikas-neutral-universal', 2: 'cetasikas-neutral-pakinnaka' });
        const badRows = mapRows([3, 4, 5, 6, 7], badRowColors, {
        ...
        const goodRows = mapRows([8, 9, 10, 11], goodRowColors, {
    ...
    const isStudyGroupExpanded = useCallback((id: string) => !collapsedStudyGroups.has(id), [collapsedStudyGroups]);
    ...
    return (<main className="page">
      ...
      <div className="mindstudy-shell">
        <aside className="mindstudy-sidebar">
          <div className="mindstudy-nav">
            ...
            <button type="button" className="mindstudy-nav-toggle" onClick={() => setNavOpen((prev) => !prev)} aria-expanded={navOpen} aria-controls="mindstudy-nav-subtopics">
              <span>Study outline</span>
              <span className={`mindstudy-caret ${navOpen ? 'open' : ''}`} aria-hidden>
                ▼
              </span>
            </button>
            <div id="mindstudy-nav-subtopics" className={`mindstudy-nav-children ${navOpen ? 'open' : ''}`} role="region" aria-label="Study outline">
            ...
            <div className="mindstudy-nav-section">
              ...
              <a className="mindstudy-nav-item sub mindstudy-nav-mental-topic" href="#cetasikas-bad" style={{ color: BAD_MENTAL_TEXT }}>
              ...
              <a className="mindstudy-nav-item sub mindstudy-nav-mental-topic" href="#cetasikas-good" style={{ color: GOOD_MENTAL_TEXT }}>
                <span className="mindstudy-nav-mental-topic-title" style={{ color: 'inherit' }}>
            ...
            <div className="mindstudy-nav-section">
              ...
              <button type="button" className="mindstudy-nav-toggle" style={{ marginTop: 6 }} onClick={() => setMindNavOpen((o) => !o)} aria-expanded={mindNavOpen} aria-controls="mindstudy-nav-mind-groups">
                <span>Citta groups</span>
                <span className={`mindstudy-caret ${mindNavOpen ? 'open' : ''}`} aria-hidden>
                  ▼
                </span>
              </button>
              <div id="mindstudy-nav-mind-groups" className={`mindstudy-nav-children ${mindNavOpen ? 'open' : ''}`} role="region" aria-label="Citta groups">
                {mindDiagramGroups.map((g) => (<a key={g.id} className="mindstudy-nav-item sub" href={`#${g.id}`}>
                    {g.subtitle || g.title}
            ...
            <div className="mindstudy-nav-section">
              ...
              {rupaCategoryBlocks.map((group) => (<a key={group.id} className="mindstudy-nav-item sub" href={`#${group.id}`}>
        ...
        <section className="mindstudy-content">
          ...
          <section className="mindstudy-topic-section" id="topic-mental" aria-labelledby="topic-mental-heading">
            ...
            return (<article key={block.id} id={block.id} className={`mindstudy-diagram-group ${mentalOpen ? '' : 'is-collapsed'}`}>
                    <button type="button" className="mindstudy-diagram-group-head mindstudy-diagram-group-toggle" aria-expanded={mentalOpen} aria-controls={`mindstudy-mental-body-${block.id}`} onClick={() => toggleStudyGroup(block.id)}>
                      <span className="mindstudy-diagram-group-dot" style={{ background: block.color }} aria-hidden/>
                      <h3 lang="en" style={{
                    color: block.id === 'cetasikas-bad'
                        ? BAD_MENTAL_TEXT
                    ...
                    <div id={`mindstudy-mental-body-${block.id}`} className={`mindstudy-diagram-group-body ${mentalOpen ? 'open' : ''}`} role="region" aria-label={`${block.titleEn} (${block.titleThai})`} aria-hidden={!mentalOpen}>
                      <div className="mindstudy-diagram-group-body-inner">
                        <div className="mindstudy-diagram-subgroups">
                          {block.rows.map((row) => (<div key={row.id} id={row.anchorId} className="mindstudy-diagram-subgroup-row">
                              <div className="mindstudy-diagram-circles compact">
                                {row.items.map((mental) => (<button key={mental.id} id={`mental-${mental.id}`} type="button" className="mindstudy-diagram-node compact" style={{ background: `${row.color}33`, borderColor: row.color, color: '#1f2937' }} onClick={() => {
          ...
          <section className="mindstudy-topic-section" id="topic-mind" aria-labelledby="topic-mind-heading">
            <header className="mindstudy-topic-section-header">
            ...
            return (<article key={group.id} id={group.id} className={`mindstudy-diagram-group ${mindGroupOpen ? '' : 'is-collapsed'}`}>
                  <button type="button" className="mindstudy-diagram-group-head mindstudy-diagram-group-toggle" aria-expanded={mindGroupOpen} aria-controls={`mindstudy-mind-body-${group.id.replace(/[^a-zA-Z0-9_-]/g, '-')}`} onClick={() => toggleStudyGroup(group.id)}>
                    ...
                    !containsThaiScript(group.subtitle || '') ? (<p lang="th">({group.title})</p>) : null}
                    <span className={`mindstudy-caret ${mindGroupOpen ? 'open' : ''}`} aria-hidden>
                      ▼
                    </span>
                  </button>
                  <div id={`mindstudy-mind-body-${group.id.replace(/[^a-zA-Z0-9_-]/g, '-')}`} className={`mindstudy-diagram-group-body ${mindGroupOpen ? 'open' : ''}`} role="region" aria-label={group.subtitle &&
                    group.title &&
                    group.title !== group.subtitle &&
                    containsThaiScript(group.title)
                    ? `${group.subtitle} (${group.title})`
                    : (group.subtitle || group.title)} aria-hidden={!mindGroupOpen}>
                  <div className="mindstudy-diagram-group-body-inner">
                  {(() => {
                        ...
                        ? [
                            {
                                ...
                                labelTh: 'โลภมูลจิต ๘',
                                color: '#b76c84',
                                match: (mindTitle, mindId, subgroup) => subgroup === 'lobha_mula' ||
                            ...
                            },
                            {
                                ...
                                labelTh: 'โทสมูลจิต ๒',
                                color: '#ef9b54',
                                match: (mindTitle, mindId, subgroup) => subgroup === 'dosa_mula' ||
                            ...
                            },
                            {
                                ...
                                labelTh: 'โมหมูลจิต ๒',
                                color: '#d9b49c',
                                match: (mindTitle, mindId, subgroup) => subgroup === 'moha_mula' ||
                            ...
                            ? [
                                {
                                    ...
                                    color: '#9fbe4c',
                                    match: (mindTitle, mindId, subgroup) => subgroup === 'ahetuka_akusala_vipaka' ||
                                ...
                                },
                                {
                                    ...
                                    color: '#a9c75a',
                                    match: (mindTitle, mindId, subgroup) => subgroup === 'ahetuka_kusala_vipaka' ||
                                ...
                                },
                                {
                                    ...
                                    color: '#8eb8dc',
                                    match: (mindTitle, mindId, subgroup) => subgroup === 'ahetuka_kiriya' ||
                                ...
                                ? [
                                    {
                                        ...
                                        color: '#ef9b54',
                                        match: (_mindTitle, mindId, subgroup) => subgroup === 'maha_kusala' ||
                                    ...
                                    },
                                    {
                                        ...
                                        color: '#9fbe4c',
                                        match: (_mindTitle, mindId, subgroup) => subgroup === 'maha_vipaka' ||
                                    ...
                                    },
                                    {
                                        ...
                                        color: '#8eb8dc',
                                        match: (_mindTitle, mindId, subgroup) => subgroup === 'maha_kiriya' ||
                                    ...
                                    ? [
                                        {
                                            ...
                                            color: '#f0b563',
                                            match: (_t, id, subgroup) => subgroup === 'rupavacara_kusala' ||
                                        ...
                                        },
                                        {
                                            ...
                                            color: '#a7c95d',
                                            match: (_t, id, subgroup) => subgroup === 'rupavacara_vipaka' ||
                                        ...
                                        },
                                        {
                                            ...
                                            color: '#8eb8dc',
                                            match: (_t, id, subgroup) => subgroup === 'rupavacara_kiriya' ||
                                        ...
                                        ? [
                                            {
                                                ...
                                                color: '#e39a73',
                                                match: (_t, id, subgroup) => subgroup === 'arupavacara_kusala' ||
                                            ...
                                            },
                                            {
                                                ...
                                                color: '#b4cd75',
                                                match: (_t, id, subgroup) => subgroup === 'arupavacara_vipaka' ||
                                            ...
                                            },
                                            {
                                                ...
                                                color: '#95bfdc',
                                                match: (_t, id, subgroup) => subgroup === 'arupavacara_kiriya' ||
                                            ...
                                            ? [
                                                {
                                                    ...
                                                    labelTh: 'มรรคจิต ๔',
                                                    color: '#e89d74',
                                                    match: (_t, id, subgroup) => subgroup === 'magga' ||
                    ...
                    if (isLokuttara) {
                        ...
                        return (<div className="mindstudy-diagram-subgroups">
                          {lokuttaraRows.map((row, rowIndex) => {
                                ...
                                if (!rowMinds.length)
                                    return null;
                                const rowMindLevels = rowMinds.flatMap((topic) => Array.from({ length: 5 }, (_, idx) => ({
                                    topic,
                                    level: idx + 1,
                                })));
                                return (<div key={row.key} className="mindstudy-diagram-subgroup-row">
                                <div className="mindstudy-diagram-circles compact">
                                  {rowMindLevels.map(({ topic, level }) => {
                                        const syntheticId = lokuttaraDisplayStart + rowIndex * 5 + (level - 1);
                                        return (<button key={`${topic.id}-level-${level}`} id={`lokuttara-${row.key}-${level}`} type="button" className="mindstudy-diagram-node compact" style={{
                    ...
                    return (<div className="mindstudy-diagram-subgroups">
                        {subgroupDefs.map((subgroup) => {
                            ...
                            if (!subgroupMinds.length)
                                return null;
                            return (<div key={subgroup.key} className="mindstudy-diagram-subgroup-row">
                              <div className="mindstudy-diagram-circles compact">
                                {subgroupMinds.map((topic, index) => {
                                    const idMatch = String(topic.id).match(/mind-(\d+)/);
                                    const nodeLabel = idMatch ? idMatch[1] : `${index + 1}`;
                                    return (<button key={topic.id} id={String(topic.id)} type="button" className="mindstudy-diagram-node compact" style={{
        ...
        <section className="mindstudy-topic-section" id="topic-rupa" aria-labelledby="topic-rupa-heading">
          <header className="mindstudy-topic-section-header">
          ...
          <div className="mindstudy-diagram-surface" id="rupa-diagram">
            ...
            return (<article key={group.id} id={group.id} className={`mindstudy-diagram-group ${rupaGroupOpen ? '' : 'is-collapsed'}`}>
                    <button type="button" className="mindstudy-diagram-group-head mindstudy-diagram-group-toggle" aria-expanded={rupaGroupOpen} aria-controls={`mindstudy-rupa-body-${group.id.replace(/[^a-zA-Z0-9_-]/g, '-')}`} onClick={() => toggleStudyGroup(group.id)}>
                      <span className="mindstudy-diagram-group-dot" style={{ background: group.color }} aria-hidden/>
                    ...
                    <div id={`mindstudy-rupa-body-${group.id.replace(/[^a-zA-Z0-9_-]/g, '-')}`} className={`mindstudy-diagram-group-body ${rupaGroupOpen ? 'open' : ''}`} role="region" aria-label={`${group.titleEn} (${group.titleTh})`} aria-hidden={!rupaGroupOpen}>
                      <div className="mindstudy-diagram-group-body-inner">
                        <div className="mindstudy-diagram-subgroups">
                          {group.rows.map((row) => (<div key={row.id} id={row.id} className="mindstudy-diagram-subgroup-row">
                              <div className="mindstudy-diagram-circles compact">
                                {row.items.map((rupa) => (<button key={rupa.id} id={`rupa-${rupa.id}`} type="button" className="mindstudy-diagram-node compact" style={{ background: `${row.color}33`, borderColor: row.color, color: '#1f2937' }} aria-label={`${rupa.name_en} (${rupa.name})`} title={`${rupa.name_en} (${rupa.pali})`} onClick={() => openRupaDetail(rupa)}>
      ...
      {showDuckPool && (<div className="mindstudy-droplet-overlay" role="dialog" aria-modal="true" aria-label="Compute water – duck pool">
          <div className="mindstudy-droplet-header">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <button type="button" className="mindstudy-pool-narrative-play" onClick={playPoolNarrative} aria-label={narrativePlaying ? 'Stop narration' : 'Play narration'} aria-pressed={narrativePlaying}>
                {narrativePlaying ? (<>
              ...
              <button type="button" className="mindstudy-modal-close" onClick={() => setShowDuckPool(false)} aria-label="Close">
                ✕
              </button>
            </div>
          </div>
          <div className="mindstudy-droplet-canvas-wrap">
            <DuckPool style={{ width: '100%', height: '100%', minHeight: 320 }} allowUserDrops={false} onDropDropletReady={(fn) => { dropDropletRef.current = fn; }}/>
            {subtitleLine !== null && (<div className="mindstudy-pool-subtitle" role="status" aria-live="polite">
          ...
          <div className="mindstudy-droplet-actions">
            ...
            <button className="mindstudy-btn primary" onClick={() => {
      ...
      {learnMoreConfirmOpen && (<div className="mindstudy-modal-backdrop" role="presentation" onClick={() => setLearnMoreConfirmOpen(false)}>
          <div className="mindstudy-modal" role="dialog" aria-modal="true" aria-label="Confirm watch" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mindstudy-modal-header">
              ...
              <button className="mindstudy-modal-close" onClick={() => setLearnMoreConfirmOpen(false)} aria-label="Close dialog">
                ✕
              </button>
            </div>
      ...
      {rupaModalOpen && selectedRupa && (<div className="mindstudy-modal-backdrop" role="presentation" onClick={() => setRupaModalOpen(false)}>
          <div className="mindstudy-modal" role="dialog" aria-modal="true" aria-label={`Inspect ${selectedRupa.name_en}`} onClick={(e) => e.stopPropagation()}>
            <div className="mindstudy-modal-glow" aria-hidden/>
            <div className="mindstudy-modal-header">
            ...
            <div className="mindstudy-modal-body">
              <h3 lang="en">
                ...
                {selectedRupa.name?.trim() ? (<span lang="th" className="mindstudy-modal-title-thai">
                    {' '}
              ...
              {selectedRupa.description?.trim() ? (<p className="mindstudy-section-desc">{selectedRupa.description.trim()}</p>) : null}
            </div>
            <div className="mindstudy-modal-actions">
      ...
      {modalOpen && selectedMind && (<div className="mindstudy-modal-backdrop" role="presentation" onClick={() => setModalOpen(false)}>
          <div className={`mindstudy-modal ${cetasikaModalOpen && selectedCetasika ? 'mindstudy-modal-with-side' : ''}`} role="dialog" aria-modal="true" aria-label={`Inspect ${selectedMind.title}`} onClick={(e) => e.stopPropagation()}>
            <div className="mindstudy-modal-glow" aria-hidden/>
            <div className="mindstudy-modal-header">
            ...
            <div className="mindstudy-modal-body">
              ...
              <ul className="mindstudy-list modal-list">
                {selectedMind.highlights.slice(0, 3).map((point) => (<li key={point}>{point}</li>))}
              </ul>
              {(() => {
                ...
                const linkedMentals = ids
                    ...
                    .filter((mental): mental is StaticMental => Boolean(mental));
                if (!linkedMentals.length)
                    return null;
                return (<div className="mindstudy-modal-mental-ids">
                    <span className="mindstudy-modal-mental-ids-label">เจตสิกในจิตนี้</span>
                    <div className="mindstudy-modal-mental-list">
                      {linkedMentals.map((mental) => (<button key={mental.id} type="button" className="mindstudy-modal-mental-item" onClick={() => openCetasikaFromStaticMental(mental)}>
                          {mental.name}
            ...
            <div className="mindstudy-modal-actions">
              ...
              <button className="mindstudy-btn primary" type="button" onClick={() => {
      ...
      {cetasikaModalOpen && selectedCetasika && (() => {
            ...
            return (<div className={sideBySideWithMind ? 'mindstudy-modal-side-shell' : 'mindstudy-modal-backdrop'} role="presentation" onClick={() => setCetasikaModalOpen(false)}>
          <div className={`mindstudy-modal ${sideBySideWithMind ? 'mindstudy-modal-side' : ''}`} role="dialog" aria-modal="true" aria-label={`Inspect ${titleEn}${thaiBracket}`} onClick={(e) => e.stopPropagation()}>
            <div className="mindstudy-modal-glow" aria-hidden/>
            <div className="mindstudy-modal-header">
            ...
            <div className="mindstudy-modal-body">
              <h3 lang="en">
                {titleEn}
                {c.thai?.trim() ? (<span lang="th" className="mindstudy-modal-title-thai">
                    {' '}
                    ({c.thai.trim()})
                  </span>) : null}
              </h3>
              ...
              {c.description?.trim() ? (<p className="mindstudy-section-desc">{c.description.trim()}</p>) : null}
              {filledRows.length > 0 ? (<dl className="mindstudy-modal-cetasika-fields">
                  {filledRows.map((row) => (<div key={row.key} className="mindstudy-modal-cetasika-field">
                      <dt>{row.label}</dt>
                      <dd lang="en">{row.text!.trim()}</dd>
                    </div>))}
                </dl>) : null}
              {c.highlights.length > 0 ? (<ul className="mindstudy-list modal-list">
                  {c.highlights.map((point) => (<li key={point}>{point}</li>))}
                </ul>) : null}
            </div>
            <div className="mindstudy-modal-actions">
              <button className="mindstudy-btn primary" onClick={() => {
