...
export function useXRSession({ renderer, overlayRoot, }: {
        ...
        (async () => {
            ...
            if (typeof xr.isSessionSupported === 'function') {
                try {
                    ...
                    if (!cancelled)
                        setVrSupport(ok ? 'supported' : 'unsupported');
                }
                catch {
                    if (!cancelled)
    ...
    const startXrSession = useCallback(async (mode: XrSessionMode) => {
        ...
        if (typeof xr.isSessionSupported === 'function') {
            try {
                ...
                if (xrModeName === 'vr')
                    setVrMessage('Failed to verify VR support');
                else
                    setArMessage('Failed to verify AR support');
                return;
            }
        }
    ...
    const vrButtonTitle = useMemo(() => {
        if (!renderer)
            return '3D renderer is still loading...';
        if (activeXrMode === 'vr')
            return 'Exit VR session';
        if (vrSupport === 'checking')
            return 'Checking VR support...';
        if (vrSupport === 'not_secure')
            return 'WebXR requires HTTPS or localhost';
        if (vrSupport === 'no_webxr')
            return 'WebXR not available in this browser/device';
        if (vrSupport === 'unsupported')
            return 'immersive-vr is not supported (no headset / not enabled)';
        return 'Enter VR';
    }, [activeXrMode, renderer, vrSupport]);
    ...
    const arButtonTitle = useMemo(() => {
        if (!renderer)
            return '3D renderer is still loading...';
        if (activeXrMode === 'ar')
            return 'Exit AR session';
        if (arSupport === 'checking')
            return 'Checking AR support...';
        if (arSupport === 'not_secure')
            return 'WebXR requires HTTPS or localhost';
        if (arSupport === 'no_webxr')
            return 'WebXR not available in this browser/device';
        if (arSupport === 'unsupported')
            return 'immersive-ar is not supported on this device/browser';
        return 'Enter AR';
    }, [activeXrMode, arSupport, renderer]);
