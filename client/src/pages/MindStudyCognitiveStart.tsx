...
const TELESCOPE_LIGHTS = [
    { position: [2, 2, 2] as [
            ...
            number,
            number
        ], intensity: 3.5, color: '#ffffff' },
    { position: [-1.5, 1, 2] as [
            number,
            number,
            number
        ], intensity: 2.5, color: '#e0f2fe' },
    { position: [0, -1, 2] as [
            number,
            number,
            number
        ], intensity: 2, color: '#fef3c7' },
];
const SENSE_OBJECTS = [
...
[telescopeModel, earModel, noseModel, tongueModel, heartModel, pictureModel, soundModel].forEach((url) => useGLTF.preload(url));
...
function SenseObjectModelPart({ senseObject }: {
...
function SenseObjectPrimitivePart({ senseObject }: {
    senseObject: SenseObject;
}) {
    return (<mesh>
      {senseObject.primitive === 'box' ? (<boxGeometry args={[0.8, 0.8, 0.8]}/>) : (<sphereGeometry args={[0.5, 32, 32]}/>)}
      <meshStandardMaterial color={senseObject.primitiveColor} roughness={0.4} metalness={0.1}/>
    </mesh>);
}
function SenseObjectScene({ senseObject }: {
    senseObject: SenseObject;
}) {
    return (<group position={[0, 0, 0]} scale={1}>
      <ambientLight intensity={senseObject.ambientBoost}/>
      {senseObject.pointLights.map((pl, i) => (<pointLight key={i} position={pl.position} intensity={pl.intensity ?? 1} color={pl.color ?? '#ffffff'} distance={8} decay={2}/>))}
      {senseObject.modelPath ? (<SenseObjectModelPart senseObject={senseObject}/>) : (<SenseObjectPrimitivePart senseObject={senseObject}/>)}
    </group>);
}
function SenseObjectCanvas({ senseObject }: {
    senseObject: SenseObject;
}): React.ReactElement {
    return (<Canvas className="mindstudy-model-canvas" style={{ width: '100%', height: 200 }} camera={{ position: [0, 0, 4], fov: 35 }}>
...
function DualCanvasBox({ internal, external }: {
    internal: SenseObject;
    external: SenseObject;
}): React.ReactElement {
    return (<div style={{ display: 'flex', gap: 8, width: '100%', minHeight: 200 }}>
...
export function MindStudyCognitiveStart(): React.ReactElement {
    ...
    return (<main className="page">
      ...
      <div className="mindstudy-shell">
        ...
        <section className="mindstudy-content">
            ...
            <div className="mindstudy-section-header" style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', marginBottom: 12 }}>
                <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                  <button type="button" className={`mindstudy-btn ${narrativeMode === 'internal' ? 'primary' : 'ghost'}`} onClick={() => setModeAndStop('internal')} aria-pressed={narrativeMode === 'internal'} style={{ padding: '8px 14px', fontSize: 13 }}>
                    อายตนะภายใน
                  </button>
                  <button type="button" className={`mindstudy-btn ${narrativeMode === 'external' ? 'primary' : 'ghost'}`} onClick={() => setModeAndStop('external')} aria-pressed={narrativeMode === 'external'} style={{ padding: '8px 14px', fontSize: 13 }}>
                    อายตนะภายนอก
                  </button>
                </div>
                <button type="button" className="mindstudy-btn primary" onClick={playNarrative} aria-label={narrativePlaying ? 'Stop narration' : 'Play narration'} aria-pressed={narrativePlaying} style={{ padding: '10px 18px', fontSize: 14, fontWeight: 700 }}>
                  {narrativePlaying ? '⏹ Stop' : '▶ Play'}
                </button>
              </div>
              {subtitleVisible && (<div role="status" aria-live="polite" style={{
                ...
                border: '1px solid rgba(15,23,42,0.1)',
                  ...
                  {subtitleLine !== null ? (<span lang="th">{subtitleLine}</span>) : (<span style={{ color: '#94a3b8' }}>Choose narrative above, then click Play</span>)}
                </div>)}
            </div>
            <div className="mindstudy-grid" style={{ gridTemplateColumns: 'repeat(3, 1fr)' }}>
              {SENSE_OBJECTS.map((obj, i) => (<article key={obj.id} className="mindstudy-card" style={narrativeMode === 'internal' && subtitleLine === obj.triggerLine
          ...
          <article className="mindstudy-section mindstudy-cognitive-box" id="cognitive">
            <button type="button" className="mindstudy-cognitive-box-trigger" onClick={() => navigate('/mind-study/cognitive')} aria-label="Go to How Cognitive process work?">
      ...
      <div style={{ padding: '0 24px 24px', maxWidth: 1200, margin: '0 auto' }}>
        <button type="button" className="mindstudy-btn ghost" onClick={() => navigate('/mind-study')} style={{ marginTop: 8 }}>
          ← Back to Mind Study
        </button>
      </div>
    </main>);
