/**
 * Abhidhamma cetasika compatibility validation.
 *
 * Operates on mental *names* (matching frontend class names and
 * static_seed_data.py entries) so the code runner can validate
 * the composition before persisting to the backend.
 */
...
export function validateMentalComposition(
  mentalNames: string[],
  ...
  if (presentBad.length > 0) {
    ...
    if (hasBad('Doubt')) {
      ...
      if (otherBad.length > 0) {
        errors.push(
          `Doubt (Vicikiccha) can only coexist with the Moha group ` +
    ...
    if (hasBad('Greed')) {
      ...
      if (dosaPresent.length > 0) {
        errors.push(
          `Greed (Lobha) cannot coexist with Dosa-group cetasika(s): ${dosaPresent.join(', ')}`,
        )
      }
    }
  }

  // Rule 4: Person type constraints on bad cetasikas
  if (personType === 'sotapanna' || personType === 'sakadagami') {
    ...
    if (presentBad.length > 0) {
      errors.push(
        `arahant cannot have ANY akusala cetasika(s): ${presentBad.join(', ')}`,
      )
    }
  }
